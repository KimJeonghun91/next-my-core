...
async function cliPrompt() {
    ...
    try {
        ...
        return { config: (_a = value === null || value === void 0 ? void 0 : value.config) !== null && _a !== void 0 ? _a : null };
    }
    catch (_b) {
        return { config: null };
    }
...
async function lint(baseDir, lintDirs, eslintrcFile, pkgJsonPath, { lintDuringBuild = false, eslintOptions = null, reportErrorsOnly = false, maxWarnings = -1, formatter = null, outputFile = null, }) {
    ...
    try {
        ...
        if (deps.missing.some((dep) => dep.pkg === 'eslint')) {
            Log.error(`ESLint must be installed${lintDuringBuild ? ' in order to run during builds:' : ':'} ${bold(cyan((packageManager === 'yarn'
                ? 'yarn add --dev'
                : packageManager === 'pnpm'
        ...
        const { ESLint } = mod;
        let eslintVersion = (_a = ESLint === null || ESLint === void 0 ? void 0 : ESLint.version) !== null && _a !== void 0 ? _a : (_b = mod.CLIEngine) === null || _b === void 0 ? void 0 : _b.version;
        ...
        let options = Object.assign({ useEslintrc: true, baseConfig: {}, errorOnUnmatchedPattern: false, extensions: ['.js', '.jsx', '.ts', '.tsx'], cache: true }, eslintOptions);
        ...
        for (const configFile of [eslintrcFile, pkgJsonPath]) {
            ...
            if ((_c = completeConfig.plugins) === null || _c === void 0 ? void 0 : _c.includes('@next/next')) {
        ...
        if (nextEslintPluginIsEnabled) {
            ...
            for (const rule of pagesDirRules) {
                if (!((_d = options.baseConfig.rules) === null || _d === void 0 ? void 0 : _d[rule]) &&
                    !((_e = options.baseConfig.rules) === null || _e === void 0 ? void 0 : _e[rule.replace('@next/next', '@next/babel-plugin-next')])) {
        ...
        if (reportErrorsOnly)
            results = await ESLint.getErrorResults(results); // Only return errors if --quiet flag is used
        if (formatter)
            selectedFormatter = await eslint.loadFormatter(formatter);
        const formattedResult = formatResults(baseDir, results, selectedFormatter === null || selectedFormatter === void 0 ? void 0 : selectedFormatter.format);
        ...
        return {
            ...
            isError: ((_f = ESLint.getErrorResults(results)) === null || _f === void 0 ? void 0 : _f.length) > 0 ||
            ...
            eventInfo: {
                ...
                lintFix: !!options.fix,
                nextEslintPluginVersion: nextEslintPluginIsEnabled && deps.resolved.has('eslint-config-next')
                    ? require(path.join(path.dirname(deps.resolved.get('eslint-config-next')), 'package.json')).version
                    : null,
...
export async function runLintCheck(baseDir, lintDirs, opts) {
    ...
    try {
        ...
        const eslintrcFile = (_a = (await findUp([
            '.eslintrc.js',
            '.eslintrc.cjs',
            '.eslintrc.yaml',
            '.eslintrc.yml',
        ...
        const pkgJsonPath = (_b = (await findUp('package.json', { cwd: baseDir }))) !== null && _b !== void 0 ? _b : null;
        ...
        if (config.exists) {
            ...
            if (lintDuringBuild) {
                ...
                const { config: selectedConfig } = strict
                    ? ESLINT_PROMPT_VALUES.find((opt) => opt.title === 'Strict')
                    : await cliPrompt();
                if (selectedConfig == null) {
                    ...
                    if (existsSync(path.join(baseDir, 'pages')) ||
