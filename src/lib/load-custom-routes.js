...
export function checkCustomRoutes(routes, type) {
    ...
    for (const route of routes) {
        if (!route || typeof route !== 'object') {
            console.error(`The route ${JSON.stringify(route)} is not a valid object with \`source\`${type !== 'middleware'
        ...
        const checkInvalidHasMissing = (items, fieldName) => {
            ...
            if (typeof items !== 'undefined' && !Array.isArray(items)) {
                ...
                for (const hasItem of items) {
                    ...
                    if (typeof hasItem.value !== 'undefined' &&
            ...
            if (route.destination.startsWith('/') &&
                ...
                if (unnamedInDest.size > 0) {
                    ...
                    if (destinationParseFailed) {
                        ...
                        const sourceSegments = new Set(sourceTokens
        ...
        if (hasInvalidKeys || hasInvalidParts) {
            console.error(`${invalidParts.join(', ')}${invalidKeys.length
                ? (hasInvalidParts ? ',' : '') +
...
function processRoutes(routes, config, type) {
    ...
    if (config.i18n && type === 'redirect') {
        for (const item of ((_a = config.i18n) === null || _a === void 0 ? void 0 : _a.domains) || []) {
            defaultLocales.push({
    ...
    for (const r of _routes) {
        ...
        const isExternal = !((_b = r.destination) === null || _b === void 0 ? void 0 : _b.startsWith('/'));
        ...
        if (config.i18n && r.locale !== false) {
            if (!isExternal) {
                defaultLocales.forEach((item) => {
                    ...
                    newRoutes.push(Object.assign(Object.assign({}, r), { destination, source: `${srcBasePath}/${item.locale}${r.source === '/' && !config.trailingSlash ? '' : r.source}` }));
                });
            }
            r.source = `/:nextInternalLocale(${config.i18n.locales
                .map((locale) => escapeStringRegexp(locale))
                .join('|')})${r.source === '/' && !config.trailingSlash ? '' : r.source}`;
            if (r.destination && ((_c = r.destination) === null || _c === void 0 ? void 0 : _c.startsWith('/'))) {
...
async function loadRedirects(config) {
    ...
    if (Array.isArray(redirects)) {
        config._originalRedirects = redirects.map((r) => (Object.assign({}, r)));
    }
...
async function loadRewrites(config) {
    ...
    if (!Array.isArray(_rewrites) &&
        ...
        Object.keys(_rewrites).every((key) => key === 'beforeFiles' || key === 'afterFiles' || key === 'fallback')) {
    ...
    config._originalRewrites = {
        beforeFiles: beforeFiles.map((r) => (Object.assign({}, r))),
        afterFiles: afterFiles.map((r) => (Object.assign({}, r))),
        fallback: fallback.map((r) => (Object.assign({}, r))),
    };
...
export default async function loadCustomRoutes(config) {
    ...
    const totalRewrites = rewrites.beforeFiles.length +
