import { accumulateViewport as originAccumulateViewport, accumulateMetadata as originAccumulateMetadata, } from './resolve-metadata';
function accumulateMetadata(metadataItems) {
    const fullMetadataItems = metadataItems.map((item) => [
        item[0],
        item[1],
        null,
    ]);
...
function accumulateViewport(viewportExports) {
    // skip the first two arguments (metadata and static metadata)
    return originAccumulateViewport(viewportExports.map((item) => [null, null, item]));
}
function mapUrlsToStrings(obj) {
...
describe('accumulateMetadata', () => {
    ...
    describe('icon', () => {
        it('should resolve icons.icon correctly', async () => {
            // array icons
            expect(await accumulateMetadata([
                [
                    {
                        icons: [
                            {
    ...
    describe('itunes', () => {
        it('should resolve relative url starting with ./ with pathname for itunes.appArgument', async () => {
            ...
            const metadata = await accumulateMetadata(metadataItems);
            expect(mapUrlsToStrings(metadata)).toMatchObject(mapUrlsToStrings({
                metadataBase: new URL('http://test.com/base'),
                itunes: {
