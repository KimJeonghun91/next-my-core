...
function mergeStaticMetadata(source, target, staticFilesMetadata, metadataContext, titleTemplates) {
    var _a, _b;
    if (!staticFilesMetadata)
        return;
    const { icon, apple, openGraph, twitter, manifest } = staticFilesMetadata;
    // file based metadata is specified and current level metadata icons is not specified
    if ((icon && !hasIconsProperty(source === null || source === void 0 ? void 0 : source.icons, 'icon')) ||
        (apple && !hasIconsProperty(source === null || source === void 0 ? void 0 : source.icons, 'apple'))) {
        target.icons = {
            icon: icon || [],
            apple: apple || [],
        };
    ...
    if (twitter && !((_a = source === null || source === void 0 ? void 0 : source.twitter) === null || _a === void 0 ? void 0 : _a.hasOwnProperty('images'))) {
        const resolvedTwitter = resolveTwitter(Object.assign(Object.assign({}, target.twitter), { images: twitter }), target.metadataBase, titleTemplates.twitter);
        target.twitter = resolvedTwitter;
    }
    // file based metadata is specified and current level metadata openGraph.images is not specified
    if (openGraph && !((_b = source === null || source === void 0 ? void 0 : source.openGraph) === null || _b === void 0 ? void 0 : _b.hasOwnProperty('images'))) {
        const resolvedOpenGraph = resolveOpenGraph(Object.assign(Object.assign({}, target.openGraph), { images: openGraph }), target.metadataBase, metadataContext, titleTemplates.openGraph);
...
function mergeMetadata({ source, target, staticFilesMetadata, titleTemplates, metadataContext, buildState, }) {
    ...
    const metadataBase = typeof (source === null || source === void 0 ? void 0 : source.metadataBase) !== 'undefined'
        ? source.metadataBase
        : target.metadataBase;
...
async function collectStaticImagesFiles(metadata, props, type) {
    ...
    return (iconPromises === null || iconPromises === void 0 ? void 0 : iconPromises.length) > 0
        ? (_a = (await Promise.all(iconPromises))) === null || _a === void 0 ? void 0 : _a.flat()
        : undefined;
}
...
export async function resolveMetadataItems({ tree, parentParams, metadataItems, errorMetadataItem, treePrefix = [], getDynamicParamFromSegment, searchParams, errorConvention, }) {
    ...
    segmentParam && segmentParam.value !== null
        ...
        parentParams;
    const layerProps = Object.assign({ params: currentParams }, (isPage && { searchParams }));
...
const hasTitle = (metadata) => { var _a; return !!((_a = metadata === null || metadata === void 0 ? void 0 : metadata.title) === null || _a === void 0 ? void 0 : _a.absolute); };
...
function postProcessMetadata(metadata, titleTemplates) {
    ...
    if (openGraph) {
        ...
        const hasTwDescription = twitter === null || twitter === void 0 ? void 0 : twitter.description;
        const hasTwImages = Boolean((twitter === null || twitter === void 0 ? void 0 : twitter.hasOwnProperty('images')) && twitter.images);
        if (!hasTwTitle)
            autoFillProps.title = openGraph.title;
        if (!hasTwDescription)
            autoFillProps.description = openGraph.description;
        if (!hasTwImages)
            autoFillProps.images = openGraph.images;
        if (Object.keys(autoFillProps).length > 0) {
            const partialTwitter = resolveTwitter(autoFillProps, metadata.metadataBase, titleTemplates.twitter);
            if (metadata.twitter) {
                metadata.twitter = Object.assign({}, metadata.twitter, Object.assign(Object.assign(Object.assign({}, (!hasTwTitle && { title: partialTwitter === null || partialTwitter === void 0 ? void 0 : partialTwitter.title })), (!hasTwDescription && {
                    description: partialTwitter === null || partialTwitter === void 0 ? void 0 : partialTwitter.description,
                })), (!hasTwImages && { images: partialTwitter === null || partialTwitter === void 0 ? void 0 : partialTwitter.images })));
            }
            else {
...
async function getMetadataFromExport(getPreloadMetadataExport, dynamicMetadataResolveState, metadataItems, currentIndex, resolvedMetadata, metadataResults) {
    ...
    if (typeof metadataExport === 'function') {
        ...
        const currentResolvedMetadata = process.env.NODE_ENV === 'development'
            ? Object.freeze(require('./clone-metadata').cloneMetadata(resolvedMetadata))
            : resolvedMetadata;
...
export async function accumulateMetadata(metadataItems, metadataContext) {
    ...
    for (let i = 0; i < metadataItems.length; i++) {
        ...
        if (i < metadataItems.length - 2) {
            titleTemplates = {
                title: ((_a = resolvedMetadata.title) === null || _a === void 0 ? void 0 : _a.template) || null,
                openGraph: ((_b = resolvedMetadata.openGraph) === null || _b === void 0 ? void 0 : _b.title.template) || null,
                twitter: ((_c = resolvedMetadata.twitter) === null || _c === void 0 ? void 0 : _c.title.template) || null,
            };
        }
    }
