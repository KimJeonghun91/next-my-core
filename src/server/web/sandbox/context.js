...
export async function clearModuleContext(path) {
    ...
    const handleContext = (key, cache, context) => {
        if (cache === null || cache === void 0 ? void 0 : cache.paths.has(path)) {
            context.delete(key);
        }
    };
...
async function createModuleContext(options) {
    ...
    const wasm = await loadWasm((_a = options.edgeFunctionEntry.wasm) !== null && _a !== void 0 ? _a : []);
...
Learn More: https://nextjs.org/docs/messages/edge-dynamic-code-evaluation`), COMPILER_NAMES.edgeServer);
            ...
            context.__next_webassembly_compile__ =
                function __next_webassembly_compile__(fn) {
                    const key = fn.toString();
...
Learn More: https://nextjs.org/docs/messages/edge-dynamic-code-evaluation`), COMPILER_NAMES.edgeServer);
            ...
            context.__next_webassembly_instantiate__ =
                async function __next_webassembly_instantiate__(fn) {
...
Learn More: https://nextjs.org/docs/messages/edge-dynamic-code-evaluation`), COMPILER_NAMES.edgeServer);
            ...
            context.fetch = async (input, init = {}) => {
                ...
                init.headers = new Headers((_a = init.headers) !== null && _a !== void 0 ? _a : {});
                const prevs = ((_b = init.headers.get(`x-middleware-subrequest`)) === null || _b === void 0 ? void 0 : _b.split(':')) || [];
                const value = prevs.concat(options.moduleName).join(':');
                init.headers.set('x-middleware-subrequest', value);
                if (!init.headers.has('user-agent')) {
                    init.headers.set(`user-agent`, `Next.js Middleware`);
                }
                const response = typeof input === 'object' && 'url' in input
                    ? __fetch(input.url, Object.assign(Object.assign(Object.assign({}, pick(input, [
                        'method',
                        'body',
                        'cache',
                        'credentials',
                        ...
                        'referrerPolicy',
                        'signal',
                    ])), init), { headers: Object.assign(Object.assign({}, Object.fromEntries(input.headers)), Object.fromEntries(init.headers)) }))
            ...
            context.Request = class extends __Request {
                constructor(input, init) {
                    const url = typeof input !== 'string' && 'url' in input
                        ? input.url
                        : String(input);
                    validateURL(url);
                    super(url, init);
                    this.next = init === null || init === void 0 ? void 0 : init.next;
                }
            };
    ...
    const evaluateInContext = (filepath) => {
        if (!moduleContext.paths.has(filepath)) {
            ...
            try {
                ...
                if (options.useCache) {
                    moduleContext === null || moduleContext === void 0 ? void 0 : moduleContext.paths.delete(filepath);
                }
                throw error;
            }
        }
    };
    return Object.assign(Object.assign({}, moduleContext), { evaluateInContext });
}
