...
describe('HeadersAdapter', () => {
    ...
    describe('sealed', () => {
        it('should be able to seal a Headers instance', () => {
            ...
            expect(() => sealed.append('x-custom-header', 'custom2')).toThrowError(ReadonlyHeadersError);
            expect(() => sealed.append('x-custom-header', 'custom2')).toThrowError(ReadonlyHeadersError);
            expect(() => sealed.delete('x-custom-header')).toThrowError(ReadonlyHeadersError);
            expect(() => sealed.set('x-custom-header', 'custom2')).toThrowError(ReadonlyHeadersError);
            expect(sealed.get('content-type')).toBe('application/json');
            expect(sealed.get('x-custom-header')).toBe('custom');
        });
