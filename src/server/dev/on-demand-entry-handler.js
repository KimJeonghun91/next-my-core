...
function getPageBundleType(pageBundlePath) {
    ...
    if (pageBundlePath === '/_error')
        return PAGE_TYPES.PAGES;
    if (isMiddlewareFilename(pageBundlePath))
...
class Invalidator {
    ...
    invalidate(compilerKeys = COMPILER_KEYS) {
        ...
        for (const key of compilerKeys) {
            ...
            (_a = this.multiCompiler.compilers[COMPILER_INDEXES[key]].watching) === null || _a === void 0 ? void 0 : _a.invalidate();
        }
    }
...
function disposeInactiveEntries(entries, maxInactiveAge) {
    Object.keys(entries).forEach((entryKey) => {
        ...
        if (isMiddlewareFilename(bundlePath) ||
...
export function onDemandEntryHandler({ hotReloader, maxInactiveAge, multiCompiler, nextConfig, pagesBufferLength, pagesDir, rootDir, appDir, }) {
    ...
    function getPagePathsFromEntrypoints(type, entrypoints) {
        ...
        for (const entrypoint of entrypoints.values()) {
            ...
            if (page) {
                const pageBundleType = ((_a = entrypoint.name) === null || _a === void 0 ? void 0 : _a.startsWith('app/'))
                    ? PAGE_TYPES.APP
                    : PAGE_TYPES.PAGES;
                pagePaths.push(getEntryKey(type, pageBundleType, page));
            }
            else if (isMiddlewareFilename(entrypoint.name) ||
    ...
    for (const compiler of multiCompiler.compilers) {
        compiler.hooks.done.tap('NextJsOnDemandEntries', () => {
            var _a;
            return (_a = getInvalidator(compiler.outputPath)) === null || _a === void 0 ? void 0 : _a.doneBuilding([
                compiler.name,
            ]);
        });
    }
    multiCompiler.hooks.done.tap('NextJsOnDemandEntries', (multiStats) => {
        ...
        const entryNames = [
            ...
            ...(edgeServerStats
                ? getPagePathsFromEntrypoints(COMPILER_NAMES.edgeServer, edgeServerStats.compilation.entrypoints)
                : []),
        ];
        ...
        (_a = getInvalidator(multiCompiler.outputPath)) === null || _a === void 0 ? void 0 : _a.doneBuilding([...COMPILER_KEYS]);
    });
