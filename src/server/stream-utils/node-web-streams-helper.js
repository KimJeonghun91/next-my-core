...
export function chainStreams(...streams) {
    ...
    for (let i = 0; i < streams.length; ++i) {
        promise = promise.then(() => streams[i].pipeTo(writable, { preventClose: i + 1 < streams.length }));
    }
    ...
    return new TransformStream({
        ...
        flush(controller) {
            ...
            if (missingTags.length > 0) {
                controller.enqueue(encoder.encode(`<script>self.__next_root_layout_missing_tags_error=${JSON.stringify({ missingTags, assetPrefix: assetPrefix !== null && assetPrefix !== void 0 ? assetPrefix : '', tree: getTree() })}</script>`));
            }
        },
    });
}
...
export async function continueFizzStream(renderStream, { suffix, inlinedDataStream, isStaticGeneration, getServerInsertedHTML, serverInsertedHTMLToHead, validateRootLayout, }) {
    ...
    return chainTransformers(renderStream, [
        ...
        getServerInsertedHTML && serverInsertedHTMLToHead
            ...
            : null,
        validateRootLayout
            ? createRootLayoutValidatorStream(validateRootLayout.assetPrefix, validateRootLayout.getTree)
            : null,
    ]);
}
