...
export class ImageOptimizerCache {
    static validateParams(req, query, nextConfig, isDev) {
        ...
        const remotePatterns = ((_a = nextConfig.images) === null || _a === void 0 ? void 0 : _a.remotePatterns) || [];
        const { url, w, q } = query;
        let href;
        if (domains.length > 0) {
        ...
        try {
            ...
            for (const file of files) {
                ...
                return {
                    ...
                    revalidateAfter: Math.max(maxAge, this.nextConfig.images.minimumCacheTTL) * 1000 +
                        Date.now(),
    ...
    async set(cacheKey, value, { revalidate, }) {
        ...
        const expireAt = Math.max(revalidate, this.nextConfig.images.minimumCacheTTL) * 1000 +
            Date.now();
        try {
...
export async function imageOptimizer(imageUpstream, paramsResult, nextConfig, isDev) {
    ...
    const upstreamType = detectContentType(upstreamBuffer) ||
        ((_a = imageUpstream.contentType) === null || _a === void 0 ? void 0 : _a.toLowerCase().trim());
    if (upstreamType) {
    ...
    else if ((upstreamType === null || upstreamType === void 0 ? void 0 : upstreamType.startsWith('image/')) &&
...
function setResponseHeaders(req, res, url, etag, contentType, isStatic, xCache, imagesConfig, maxAge, isDev) {
    ...
    res.setHeader('Content-Disposition', contentDisposition(fileName, { type: imagesConfig.contentDispositionType }));
