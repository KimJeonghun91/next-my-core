    ...
    class ProxiedLanguageServiceHost {
        constructor() {
            ...
            this.getScriptVersion = (fileName) => {
                ...
                if (!file)
                    return languageServiceHost.getScriptVersion(fileName);
...
function updateVirtualFileWithType(fileName, node, isGenerateMetadata) {
    ...
    if (ts.isFunctionDeclaration(node)) {
        if (isGenerateMetadata) {
            ...
            const isAsync = (_a = node.modifiers) === null || _a === void 0 ? void 0 : _a.some((m) => m.kind === ts.SyntaxKind.AsyncKeyword);
    ...
    const newSource = sourceText.slice(0, nodeEnd) +
        annotation +
...
const metadata = {
    filterCompletionsAtPosition(fileName, position, _options, prior) {
        ...
        if (completions) {
            ...
            completions.entries = completions.entries
                ...
                .map((e) => {
                const insertText = e.kind === ts.ScriptElementKind.memberVariableElement &&
    ...
    getSemanticDiagnosticsForExportVariableStatementInClientEntry(fileName, node) {
        ...
        if (ts.isFunctionDeclaration(node)) {
            if (((_a = node.name) === null || _a === void 0 ? void 0 : _a.getText()) === 'generateMetadata') {
                return [
                    {
                        file: source,
