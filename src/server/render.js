...
function renderPageTree(App, Component, props) {
    return _jsx(App, Object.assign({ Component: Component }, props));
}
...
export function errorToJSON(err) {
    ...
    if (process.env.NEXT_RUNTIME !== 'edge') {
        source =
            require('next/dist/compiled/@next/react-dev-overlay/dist/middleware').getErrorSource(err) || 'server';
    }
    return {
        name: err.name,
        source,
...
export async function renderToHTMLImpl(req, res, pathname, query, renderOpts, extra) {
    ...
    setLazyProp({ req: req }, 'cookies', getCookieParser(req.headers));
    const metadata = {};
    ...
    const hasPageGetInitialProps = !!(Component === null || Component === void 0 ? void 0 : Component.getInitialProps);
    const hasPageScripts = Component === null || Component === void 0 ? void 0 : Component.unstable_scriptLoader;
    const pageIsDynamic = isDynamicRoute(pathname);
    const defaultErrorGetInitialProps = pathname === '/_error' &&
        Component.getInitialProps ===
            Component.origGetInitialProps;
    if (renderOpts.nextExport &&
        hasPageGetInitialProps &&
        !defaultErrorGetInitialProps) {
        warn(`Detected getInitialProps on page '${pathname}'` +
    ...
    let isAutoExport = !hasPageGetInitialProps &&
    ...
    for (const methodName of [
        ...
        if (Component === null || Component === void 0 ? void 0 : Component[methodName]) {
    ...
    const routerIsReady = !!(getServerSideProps ||
        hasPageGetInitialProps ||
    ...
    const AppContainer = ({ children }) => (_jsx(AppRouterContext.Provider, { value: appRouter, children: _jsx(SearchParamsContext.Provider, { value: adaptForSearchParams(router), children: _jsx(PathnameContextProviderAdapter, { router: router, isAutoExport: isAutoExport, children: _jsx(PathParamsContext.Provider, { value: adaptForPathParams(router), children: _jsx(RouterContext.Provider, { value: router, children: _jsx(AmpStateContext.Provider, { value: ampState, children: _jsx(HeadManagerContext.Provider, { value: {
                                    updateHead: (state) => {
                                        head = state;
                                    },
                                ...
                                }, children: _jsx(LoadableContext.Provider, { value: (moduleName) => reactLoadableModules.push(moduleName), children: _jsx(StyleRegistry, { registry: jsxStyleRegistry, children: _jsx(ImageConfigContext.Provider, { value: images, children: children }) }) }) }) }) }) }) }) }) }));
    ...
    const AppContainerWithIsomorphicFiberStructure = ({ children }) => {
        return (_jsxs(_Fragment, { children: [_jsx(Noop, {}), _jsx(AppContainer, { children: _jsxs(_Fragment, { children: [dev ? (_jsxs(_Fragment, { children: [children, _jsx(Noop, {})] })) : (children), _jsx(Noop, {})] }) })] }));
    };
    const ctx = {
        err,
        ...
        AppTree: (props) => {
            return (_jsx(AppContainerWithIsomorphicFiberStructure, { children: renderPageTree(App, OriginComponent, Object.assign(Object.assign({}, props), { router })) }));
        },
        defaultGetInitialProps: async (docCtx, options = {}) => {
            const enhanceApp = (AppComp) => {
                return (props) => _jsx(AppComp, Object.assign({}, props));
            };
    ...
    if (isSSG && !isFallback) {
        ...
        try {
            ...
            }, () => getStaticProps(Object.assign(Object.assign(Object.assign({}, (pageIsDynamic
                ? { params: query }
                : undefined)), (isPreview
                ? { draftMode: true, preview: true, previewData: previewData }
                : undefined)), { locales: renderOpts.locales, locale: renderOpts.locale, defaultLocale: renderOpts.defaultLocale })));
        }
        ...
        const invalidKeys = Object.keys(data).filter((key) => key !== 'revalidate' &&
            key !== 'props' &&
            key !== 'redirect' &&
            key !== 'notFound');
    ...
    if (getServerSideProps && !isFallback) {
        ...
        try {
            ...
            }, async () => getServerSideProps(Object.assign(Object.assign(Object.assign({ req: req, res: resOrProxy, query, resolvedUrl: renderOpts.resolvedUrl }, (pageIsDynamic
                ? { params: params }
                : undefined)), (previewData !== false
                ? { draftMode: true, preview: true, previewData: previewData }
                : undefined)), { locales: renderOpts.locales, locale: renderOpts.locale, defaultLocale: renderOpts.defaultLocale })));
            canAccessRes = false;
        }
            ...
            // remove not found error code to prevent triggering legacy
            // 404 rendering
            if (isError(serverSidePropsError) &&
    ...
    if (isAutoExport && pageIsDynamic) {
        ...
        if (page in filteredBuildManifest.pages) {
            filteredBuildManifest = Object.assign(Object.assign({}, filteredBuildManifest), { pages: Object.assign(Object.assign({}, filteredBuildManifest.pages), { [page]: [
    ...
    const Body = ({ children }) => {
        return inAmpMode ? children : _jsx("div", { id: "__next", children: children });
    };
    const renderDocument = async () => {
        ...
        async function loadDocumentInitialProps(renderShell) {
            const renderPage = async (options = {}) => {
                if (ctx.err && ErrorDebug) {
                    ...
                    const html = await renderToString(_jsx(Body, { children: _jsx(ErrorDebug, { error: ctx.err }) }));
                    return { html, head };
                }
                ...
                const html = await renderToString(_jsx(Body, { children: _jsx(AppContainerWithIsomorphicFiberStructure, { children: renderPageTree(EnhancedApp, EnhancedComponent, Object.assign(Object.assign({}, props), { router })) }) }));
                return { html, head };
            };
        ...
        const renderContent = (_App, _Component) => {
            ...
            return ctx.err && ErrorDebug ? (_jsx(Body, { children: _jsx(ErrorDebug, { error: ctx.err }) })) : (_jsx(Body, { children: _jsx(AppContainerWithIsomorphicFiberStructure, { children: renderPageTree(EnhancedApp, EnhancedComponent, Object.assign(Object.assign({}, props), { router })) }) }));
        };
        ...
        const createBodyResult = getTracer().wrap(RenderSpan.createBodyResult, (initialStream, suffix) => {
            return continueFizzStream(initialStream, {
                suffix,
                inlinedDataStream: serverComponentsInlinedTransformStream === null || serverComponentsInlinedTransformStream === void 0 ? void 0 : serverComponentsInlinedTransformStream.readable,
        ...
        const documentElement = (htmlProps) => {
            if (process.env.NEXT_RUNTIME === 'edge') {
                ...
                return _jsx(Document, Object.assign({}, htmlProps, docProps));
            }
        };
        let styles;
        if (hasDocumentGetInitialProps) {
    ...
    (_a = getTracer().getRootSpanAttributes()) === null || _a === void 0 ? void 0 : _a.set('next.route', renderOpts.page);
    ...
    const htmlProps = {
        __NEXT_DATA__: {
            ...
            isFallback,
            isExperimentalCompile,
            dynamicIds: dynamicImportsIds.size === 0
                ? undefined
        ...
        canonicalBase: !renderOpts.ampPath && getRequestMeta(req, 'didStripLocale')
    ...
    const document = (_jsx(AmpStateContext.Provider, { value: ampState, children: _jsx(HtmlContext.Provider, { value: htmlProps, children: documentResult.documentElement(htmlProps) }) }));
