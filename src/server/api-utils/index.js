    ...
    res.setHeader(`Set-Cookie`, [
        ...(typeof previous === 'string'
            ...
            : Array.isArray(previous)
                ? previous
                : []),
        serialize(COOKIE_NAME_PRERENDER_BYPASS, '', Object.assign({ 
            // To delete a cookie, set `expires` to a date in the past:
            // https://tools.ietf.org/html/rfc6265#section-4.1.1
            // `Max-Age: 0` is not valid, thus ignored, and the cookie is persisted.
            expires: new Date(0), httpOnly: true, sameSite: process.env.NODE_ENV !== 'development' ? 'none' : 'lax', secure: process.env.NODE_ENV !== 'development', path: '/' }, (options.path !== undefined
            ? { path: options.path }
            : undefined))),
        serialize(COOKIE_NAME_PRERENDER_DATA, '', Object.assign({ 
            // To delete a cookie, set `expires` to a date in the past:
            // https://tools.ietf.org/html/rfc6265#section-4.1.1
            // `Max-Age: 0` is not valid, thus ignored, and the cookie is persisted.
            expires: new Date(0), httpOnly: true, sameSite: process.env.NODE_ENV !== 'development' ? 'none' : 'lax', secure: process.env.NODE_ENV !== 'development', path: '/' }, (options.path !== undefined
