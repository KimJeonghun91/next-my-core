...
export async function startServer(serverOptions) {
    ...
    async function requestListener(req, res) {
        try {
            ...
            if (isDev) {
                if (v8.getHeapStatistics().used_heap_size >
    ...
    await new Promise((resolve) => {
        server.on('listening', async () => {
            ...
            try {
                ...
                const startServerProcessDuration = performance.mark('next-start-end') &&
                    performance.measure('next-start-duration', 'next-start', 'next-start-end').duration;
                handlersReady();
                const formatDurationText = startServerProcessDuration > 2000
                    ? `${Math.round(startServerProcessDuration / 100) / 10}s`
                    : `${Math.round(startServerProcessDuration)}ms`;
                Log.event(`Ready in ${formatDurationText}`);
                if (process.env.TURBOPACK) {
                    await validateTurboNextConfig(Object.assign(Object.assign({}, serverOptions), { isDev: true }));
                }
            }
            catch (err) {
