...
export function logStartInfo({ networkUrl, appUrl, envInfo, expFeatureInfo, maxExperimentalFeatures, }) {
    Log.bootstrap(bold(purple(` ${Log.prefixes.ready} Next.js ${process.env.__NEXT_VERSION}${process.env.TURBOPACK ? ' (turbo)' : ''}`)));
    if (appUrl) {
    ...
    if (envInfo === null || envInfo === void 0 ? void 0 : envInfo.length)
        Log.bootstrap(` - Environments: ${envInfo.join(', ')}`);
    if (expFeatureInfo === null || expFeatureInfo === void 0 ? void 0 : expFeatureInfo.length) {
