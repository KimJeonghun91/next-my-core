...
export async function decodeBuffer(_buffer) {
    ...
    const firstChunkString = Array.from(firstChunk)
        ...
        .join('');
    const key = (_a = Object.entries(supportedFormats).find(([, { detectors }]) => detectors.some((detector) => detector.exec(firstChunkString)))) === null || _a === void 0 ? void 0 : _a[0];
    if (!key) {
...
export async function resize({ image, width, height }) {
    ...
    return await m(image.data, image.width, image.height, Object.assign(Object.assign({}, p.defaultOptions), { width,
        height }));
}
export async function encodeJpeg(image, { quality }) {
    ...
    const m = await e.enc();
    const r = await m.encode(image.data, image.width, image.height, Object.assign(Object.assign({}, e.defaultEncoderOptions), { quality }));
...
export async function encodeAvif(image, { quality }) {
    ...
    const r = await m.encode(image.data, image.width, image.height, Object.assign(Object.assign({}, e.defaultEncoderOptions), { 
...
export async function encodePng(image) {
    ...
    const m = await e.enc();
    const r = await m.encode(image.data, image.width, image.height, Object.assign({}, e.defaultEncoderOptions));
    return Buffer.from(r);
}
