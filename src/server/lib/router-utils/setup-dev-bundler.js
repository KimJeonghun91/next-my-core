...
async function startWatcher(opts) {
    ...
    async function propagateServerField(field, args) {
        var _a, _b;
        await ((_b = (_a = opts.renderServer) === null || _a === void 0 ? void 0 : _a.instance) === null || _b === void 0 ? void 0 : _b.propagateServerField(opts.dir, field, args));
    }
    const serverFields = {};
    async function createHotReloaderTurbopack() {
        ...
        const hasRewrites = opts.fsChecker.rewrites.afterFiles.length > 0 ||
        ...
        const project = await bindings.turbo.createProject({
            ...
            jsConfig: jsConfig !== null && jsConfig !== void 0 ? jsConfig : { compilerOptions: {} },
            watch: true,
            env: process.env,
        ...
        function processIssues(name, result, throwIssue = false) {
            ...
            for (const issue of result.issues) {
                ...
                if (/(^|\/)node_modules(\/|$)/.test(issue.filePath))
                    continue;
        ...
        async function processResult(id, result) {
            ...
            for (const { path: p, contentHash } of result.serverPaths) {
                ...
                if ((localHash && localHash !== contentHash) ||
        ...
        async function loadPartialManifest(name, pageName, type = 'pages') {
            const manifestPath = path.posix.join(distDir, `server`, type === 'app-route' ? 'app' : type, type === 'middleware' || type === 'instrumentation'
                ? ''
                : pageName === '/'
                    ? 'index'
                    : pageName === '/index' || pageName.startsWith('/index/')
                        ? `/index${pageName}`
                        : pageName, type === 'app' ? 'page' : type === 'app-route' ? 'route' : '', name);
        ...
        async function changeSubscription(page, type, includeIssues, endpoint, makePayload) {
            var _a, e_1, _b, _c;
            const key = `${page} (${type})`;
            if (!endpoint || changeSubscriptions.has(key))
                return;
            const changedPromise = endpoint[`${type}Changed`](includeIssues);
            changeSubscriptions.set(key, changedPromise);
            const changed = await changedPromise;
            try {
                for (var _d = true, changed_1 = __asyncValues(changed), changed_1_1; changed_1_1 = await changed_1.next(), _a = changed_1_1.done, !_a; _d = true) {
                    _c = changed_1_1.value;
                    _d = false;
                    const change = _c;
                ...
                try {
                    if (!_d && !_a && (_b = changed_1.return)) await _b.call(changed_1);
                }
        ...
        function mergeBuildManifests(manifests) {
            ...
            for (const m of manifests) {
                ...
                if (m.rootMainFiles.length)
                    manifest.rootMainFiles = m.rootMainFiles;
            }
            return manifest;
        }
        ...
        function mergeMiddlewareManifests(manifests) {
            ...
            const updateFunctionDefinition = (fun) => {
                var _a;
                return Object.assign(Object.assign({}, fun), { files: [...((_a = instrumentation === null || instrumentation === void 0 ? void 0 : instrumentation.files) !== null && _a !== void 0 ? _a : []), ...fun.files] });
            };
        ...
        async function mergeActionManifests(manifests) {
            ...
            function mergeActionIds(actionEntries, other) {
                ...
                for (const key in other) {
                    const action = ((_a = actionEntries[key]) !== null && _a !== void 0 ? _a : (actionEntries[key] = {
                        workers: {},
                        layer: {},
                    }));
        ...
        async function writeBuildManifest(rewrites) {
            ...
            const content = Object.assign(Object.assign({ __rewrites: rewrites
                    ? normalizeRewritesForBuildManifest(rewrites)
                    : { afterFiles: [], beforeFiles: [], fallback: [] } }, Object.fromEntries([...curEntries.keys()].map((pathname) => [
                pathname,
        ...
        async function subscribeToHmrEvents(id, client) {
            ...
            try {
                ...
                try {
                    for (var _d = true, subscription_1 = __asyncValues(subscription), subscription_1_1; subscription_1_1 = await subscription_1.next(), _a = subscription_1_1.done, !_a; _d = true) {
                    ...
                    try {
                        if (!_d && !_a && (_b = subscription_1.return)) await _b.call(subscription_1);
                    }
        ...
        function unsubscribeToHmrEvents(id, client) {
            ...
            const subscription = mapping === null || mapping === void 0 ? void 0 : mapping.get(id);
        ...
        try {
            async function handleEntries() {
                ...
                var _d;
                try {
                    for (var _e = true, iter_1 = __asyncValues(iter), iter_1_1; iter_1_1 = await iter_1.next(), _a = iter_1_1.done, !_a; _e = true) {
                        _c = iter_1_1.value;
                        _e = false;
                        ...
                        for (const [pathname, subscriptionPromise] of changeSubscriptions) {
                            ...
                            if (!curEntries.has(pathname)) {
                                ...
                                (_d = subscription.return) === null || _d === void 0 ? void 0 : _d.call(subscription);
                        ...
                        if (opts.nextConfig.experimental.instrumentationHook &&
                            instrumentation) {
                        ...
                        if (middleware) {
                            const processMiddleware = async () => {
                                ...
                                serverFields.middleware = {
                                    match: null,
                                    page: '/',
                                    matchers: (_a = middlewareManifests.get('middleware')) === null || _a === void 0 ? void 0 : _a.middleware['/'].matchers,
                                };
                            };
                    ...
                    try {
                        if (!_e && !_a && (_b = iter_1.return)) await _b.call(iter_1);
                    }
        ...
        if (process.env.NEXT_HMR_TIMING) {
            ...
            (async (proj) => {
                var _a, e_4, _b, _c;
                try {
                    for (var _d = true, _e = __asyncValues(proj.updateInfoSubscribe()), _f; _f = await _e.next(), _a = _f.done, !_a; _d = true) {
                        _c = _f.value;
                        _d = false;
                        const updateInfo = _c;
                        if (hmrEventHappened) {
        ...
        const hotReloader = {
            ...
            async run(req, res, _parsedUrl) {
                ...
                if ((_a = req.url) === null || _a === void 0 ? void 0 : _a.startsWith('/_next/static/chunks/pages/')) {
                        ...
                        switch (parsedData.event) {
                            ...
                            case 'span-end': {
                                new Span({
                                    ...
                                    startTime: BigInt(Math.floor(parsedData.startTime)) *
                                        BigInt(MILLISECONDS_IN_NANOSECOND),
                                    attrs: parsedData.attributes,
                                }).stop(BigInt(Math.floor(parsedData.endTime)) *
                                    BigInt(MILLISECONDS_IN_NANOSECOND));
                                break;
                            }
            ...
            async getCompilationErrors(page) {
                ...
                if (thisPageIssues !== undefined && thisPageIssues.size > 0) {
                    ...
                    return [...thisPageIssues.values()].map((issue) => new Error(formatIssue(issue)));
                }
            ...
            async ensurePage({ page: inputPage, 
            // Unused parameters
            // clientOnly,
            // appPaths,
            definition, isApp, url: requestUrl, }) {
                var _a, _b, _c;
                let page = (_a = definition === null || definition === void 0 ? void 0 : definition.pathname) !== null && _a !== void 0 ? _a : inputPage;
                if (page === '/_error') {
                ...
                await currentEntriesHandling;
                const route = (_b = curEntries.get(page)) !== null && _b !== void 0 ? _b : curEntries.get(normalizeAppPath(normalizeMetadataRoute((_c = definition === null || definition === void 0 ? void 0 : definition.page) !== null && _c !== void 0 ? _c : inputPage)));
                if (!route) {
                    ...
                    if (page === '/middleware')
                        return;
                    if (page === '/src/middleware')
                        return;
                    if (page === '/instrumentation')
                        return;
                    if (page === '/src/instrumentation')
                        return;
                    ...
                    switch (route.type) {
                        case 'page': {
                            ...
                            try {
                                ...
                                const type = writtenEndpoint === null || writtenEndpoint === void 0 ? void 0 : writtenEndpoint.type;
    ...
    await new Promise(async (resolve, reject) => {
        ...
        const wp = new Watchpack({
            ignored: (pathname) => {
                return (!files.some((file) => file.startsWith(pathname)) &&
        ...
        wp.on('aggregated', async () => {
            ...
            pageFiles.clear();
            devPageFiles.clear();
            const sortedKnownFiles = [...knownFiles.keys()].sort(sortByPageExts(nextConfig.pageExtensions));
            for (const fileName of sortedKnownFiles) {
                ...
                const watchTimeChange = watchTime === undefined ||
                    (watchTime && watchTime !== (meta === null || meta === void 0 ? void 0 : meta.timestamp));
                fileWatchTimes.set(fileName, meta === null || meta === void 0 ? void 0 : meta.timestamp);
                ...
                if ((meta === null || meta === void 0 ? void 0 : meta.accuracy) === undefined ||
                    !validFileMatcher.isPageFile(fileName)) {
                    continue;
                }
                const isAppPath = Boolean(appDir &&
                    normalizePathSep(fileName).startsWith(normalizePathSep(appDir) + '/'));
                const isPagePath = Boolean(pagesDir &&
                    normalizePathSep(fileName).startsWith(normalizePathSep(pagesDir) + '/'));
                ...
                if (isMiddlewareFile(rootFile)) {
                    ...
                    await propagateServerField('actualMiddlewareFile', serverFields.actualMiddlewareFile);
                    middlewareMatchers = ((_a = staticInfo.middleware) === null || _a === void 0 ? void 0 : _a.matchers) || [
            ...
            if (nextConfig.experimental.clientRouterFilter) {
                clientRouterFilters = createClientRouterFilter(Object.keys(appPaths), nextConfig.experimental.clientRouterFilterRedirects
                    ? (nextConfig._originalRedirects || []).filter((r) => !r.internal)
                    : [], nextConfig.experimental.clientRouterFilterAllowedRate);
                if (!previousClientRouterFilters ||
            ...
            if (envChange || tsconfigChange) {
                ...
                (_b = hotReloader.activeWebpackConfigs) === null || _b === void 0 ? void 0 : _b.forEach((config, idx) => {
                    var _a, _b, _c;
                    ...
                    if (tsconfigChange) {
                        (_b = (_a = config.resolve) === null || _a === void 0 ? void 0 : _a.plugins) === null || _b === void 0 ? void 0 : _b.forEach((plugin) => {
                            ...
                            if (plugin && plugin.jsConfigPlugin && tsconfigResult) {
                                ...
                                const resolvedUrlIndex = (_b = (_a = config.resolve) === null || _a === void 0 ? void 0 : _a.modules) === null || _b === void 0 ? void 0 : _b.findIndex((item) => item === currentResolvedBaseUrl);
                                if (resolvedBaseUrl) {
                                    if (resolvedBaseUrl.baseUrl !== currentResolvedBaseUrl.baseUrl) {
                                        // remove old baseUrl and add new one
                                        if (resolvedUrlIndex && resolvedUrlIndex > -1) {
                                            (_d = (_c = config.resolve) === null || _c === void 0 ? void 0 : _c.modules) === null || _d === void 0 ? void 0 : _d.splice(resolvedUrlIndex, 1);
                                        }
                                        // If the resolvedBaseUrl is implicit we only remove the previous value.
                                        // Only add the baseUrl if it's explicitly set in tsconfig/jsconfig
                                        if (!resolvedBaseUrl.isImplicit) {
                                            (_f = (_e = config.resolve) === null || _e === void 0 ? void 0 : _e.modules) === null || _f === void 0 ? void 0 : _f.push(resolvedBaseUrl.baseUrl);
                                        }
                                    }
                                }
                                if (((_g = jsConfig === null || jsConfig === void 0 ? void 0 : jsConfig.compilerOptions) === null || _g === void 0 ? void 0 : _g.paths) && resolvedBaseUrl) {
            ...
            opts.fsChecker.middlewareMatcher = ((_c = serverFields.middleware) === null || _c === void 0 ? void 0 : _c.matchers)
                ? getMiddlewareRouteMatcher((_d = serverFields.middleware) === null || _d === void 0 ? void 0 : _d.matchers)
                : undefined;
            opts.fsChecker.interceptionRoutes =
                ((_e = generateInterceptionRoutesRewrites(Object.keys(appPaths), opts.nextConfig.basePath)) === null || _e === void 0 ? void 0 : _e.map((item) => buildCustomRoute('before_files_rewrite', item, opts.nextConfig.basePath, opts.nextConfig.experimental.caseSensitiveRoutes))) || [];
            const exportPathMap = (typeof nextConfig.exportPathMap === 'function' &&
                (await ((_f = nextConfig.exportPathMap) === null || _f === void 0 ? void 0 : _f.call(nextConfig, {}, {
                    dev: true,
                    dir: opts.dir,
                    outDir: null,
                    distDir: distDir,
            ...
            try {
                ...
                for (const page of sortedRoutes) {
                    ...
                    dataRoutes.push(Object.assign(Object.assign({}, route), { regex: routeRegex.re.toString(), match: getRouteMatcher({
                            // TODO: fix this in the manifest itself, must also be fixed in
                            // upstream builder that relies on this
                            re: opts.nextConfig.i18n
                                ? new RegExp(route.dataRouteRegex.replace(`/development/`, `/development/(?<nextLocale>[^/]+?)/`))
                                : new RegExp(route.dataRouteRegex),
                            groups: routeRegex.groups,
                        }) }));
                }
                opts.fsChecker.dynamicRoutes.unshift(...dataRoutes);
                if (!(prevSortedRoutes === null || prevSortedRoutes === void 0 ? void 0 : prevSortedRoutes.every((val, idx) => val === sortedRoutes[idx]))) {
    ...
    async function requestHandler(req, res) {
        ...
        if ((_a = parsedUrl.pathname) === null || _a === void 0 ? void 0 : _a.includes(clientPagesManifestPath)) {
            res.statusCode = 200;
        ...
        if ((_b = parsedUrl.pathname) === null || _b === void 0 ? void 0 : _b.includes(devMiddlewareManifestPath)) {
            res.statusCode = 200;
            res.setHeader('Content-Type', 'application/json; charset=utf-8');
            res.end(JSON.stringify(((_c = serverFields.middleware) === null || _c === void 0 ? void 0 : _c.matchers) || []));
            return { finished: true };
        }
    ...
    async function logErrorWithOriginalStack(err, type) {
        ...
        if (isError(err) && err.stack) {
            try {
                ...
                const frame = frames.find(({ file }) => !(file === null || file === void 0 ? void 0 : file.startsWith('eval')) &&
                    !(file === null || file === void 0 ? void 0 : file.includes('web/adapter')) &&
                    !(file === null || file === void 0 ? void 0 : file.includes('web/globals')) &&
                    !(file === null || file === void 0 ? void 0 : file.includes('sandbox/context')) &&
                    !(file === null || file === void 0 ? void 0 : file.includes('<anonymous>')));
                let originalFrame, isEdgeCompiler;
                const frameFile = frame === null || frame === void 0 ? void 0 : frame.file;
                if ((frame === null || frame === void 0 ? void 0 : frame.lineNumber) && frameFile) {
                    if (hotReloader.turbopackProject) {
                        try {
                            originalFrame = await createOriginalTurboStackFrame(hotReloader.turbopackProject, {
                                file: frameFile,
                                methodName: frame.methodName,
                                line: (_a = frame.lineNumber) !== null && _a !== void 0 ? _a : 0,
                                column: frame.column,
                                isServer: true,
                            });
                        }
                        ...
                        const compilation = (isEdgeCompiler
                            ? (_b = hotReloader.edgeServerStats) === null || _b === void 0 ? void 0 : _b.compilation
                            : (_c = hotReloader.serverStats) === null || _c === void 0 ? void 0 : _c.compilation);
                        const source = await getSourceById(!!((_d = frame.file) === null || _d === void 0 ? void 0 : _d.startsWith(path.sep)) ||
                            !!((_e = frame.file) === null || _e === void 0 ? void 0 : _e.startsWith('file:')), moduleId, compilation);
                        try {
                            originalFrame = await createOriginalStackFrame({
                                ...
                                serverCompilation: isEdgeCompiler
                                    ? undefined
                                    : (_f = hotReloader.serverStats) === null || _f === void 0 ? void 0 : _f.compilation,
                                edgeCompilation: isEdgeCompiler
                                    ? (_g = hotReloader.edgeServerStats) === null || _g === void 0 ? void 0 : _g.compilation
                                    : undefined,
                            });
                        }
                        catch (_j) { }
    ...
    return {
        ...
        async ensureMiddleware(requestUrl) {
            if (!serverFields.actualMiddlewareFile)
                return;
