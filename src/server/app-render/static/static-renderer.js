class StaticRenderer {
    constructor(options) {
        ...
        this.prerender = (process.env.__NEXT_EXPERIMENTAL_REACT
...
class StaticResumeRenderer {
    constructor(postponed, options) {
        ...
        this.options = options;
        this.resume = require('react-dom/server.edge')
            .resume;
    }
    async render(children) {
...
export class ServerRenderer {
    constructor(options) {
        this.options = options;
        this.renderToReadableStream = require('react-dom/server.edge')
