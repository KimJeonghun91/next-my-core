...
async function generateFlight(ctx, options) {
    ...
    if (!(options === null || options === void 0 ? void 0 : options.skipFlight)) {
        ...
        flightData = (await walkTreeWithFlightRouterState({
            ...
            _jsx(MetadataTree, {}, requestId)),
            injectedCSS: new Set(),
            injectedJS: new Set(),
            injectedFontPreloadTags: new Set(),
            rootLayoutIncluded: false,
            asNotFound: ctx.isNotFoundPath || (options === null || options === void 0 ? void 0 : options.asNotFound),
            metadataOutlet: _jsx(MetadataOutlet, {}),
...
async function ReactServerApp({ tree, ctx, preinitScripts, asNotFound, }) {
    ...
    return (_jsxs(_Fragment, { children: [styles, _jsx(AppRouter, { buildId: ctx.renderOpts.buildId, assetPrefix: ctx.assetPrefix, initialCanonicalUrl: urlPathname, 
                // This is the router state tree.
                initialTree: initialTree, 
                // This is the tree of React nodes that are seeded into the cache
                initialSeedData: seedData, initialHead: _jsxs(_Fragment, { children: [ctx.res.statusCode > 400 && (_jsx("meta", { name: "robots", content: "noindex" })), _jsx(MetadataTree, {}, ctx.requestId)] }), globalErrorComponent: GlobalError, 
...
async function ReactServerError({ tree, ctx, preinitScripts, errorType, }) {
    ...
    const head = (_jsxs(_Fragment, { children: [_jsx(MetadataTree, {}, requestId), res.statusCode >= 400 && _jsx("meta", { name: "robots", content: "noindex" }), process.env.NODE_ENV === 'development' && (_jsx("meta", { name: "next-error", content: "not-found" }))] }));
    const initialTree = createFlightRouterStateFromLoaderTree(tree, getDynamicParamFromSegment, query);
    ...
    const initialSeedData = [
        initialTree[0],
        {},
        _jsxs("html", { id: "__next_error__", children: [_jsx("head", {}), _jsx("body", {})] }),
    ];
    return (_jsx(AppRouter, { buildId: ctx.renderOpts.buildId, assetPrefix: ctx.assetPrefix, initialCanonicalUrl: urlPathname, initialTree: initialTree, initialHead: head, globalErrorComponent: GlobalError, initialSeedData: initialSeedData, missingSlots: new Set() }));
}
...
async function renderToHTMLOrFlightImpl(req, res, pagePath, query, renderOpts, baseCtx) {
    ...
    const metadata = {};
    const appUsingSizeAdjustment = !!(nextFontManifest === null || nextFontManifest === void 0 ? void 0 : nextFontManifest.appUsingSizeAdjust);
    ...
    const isPrefetchRSCRequest = isRSCRequest &&
        req.headers[NEXT_ROUTER_PREFETCH_HEADER.toLowerCase()] !== undefined;
    /**
     * Router state provided from the client-side router. Used to handle rendering from the common layout down.
     */
    let providedFlightRouterState = isRSCRequest && (!isPrefetchRSCRequest || !renderOpts.experimental.ppr)
        ? parseAndValidateFlightRouterState(req.headers[NEXT_ROUTER_STATE_TREE.toLowerCase()])
        : undefined;
    /**
     ...
     */
    const params = (_a = renderOpts.params) !== null && _a !== void 0 ? _a : {};
    const getDynamicParamFromSegment = makeGetDynamicParamFromSegment(params, providedFlightRouterState);
    const ctx = Object.assign(Object.assign({}, baseCtx), { getDynamicParamFromSegment,
        query, isPrefetch: isPrefetchRSCRequest, providedSearchParams,
        requestTimestamp,
        searchParamsProps,
        appUsingSizeAdjustment,
        providedFlightRouterState,
        requestId, defaultRevalidate: false, pagePath,
        clientReferenceManifest,
        assetPrefix,
    ...
    (_b = getTracer().getRootSpanAttributes()) === null || _b === void 0 ? void 0 : _b.set('next.route', pagePath);
    ...
    const renderToStream = getTracer().wrap(AppRenderSpan.getBodyResult, {
        ...
        const polyfills = buildManifest.polyfillFiles
            ...
            integrity: subresourceIntegrityManifest === null || subresourceIntegrityManifest === void 0 ? void 0 : subresourceIntegrityManifest[polyfill],
        ...
        const serverComponentsRenderer = createReactServerRenderer(_jsx(ReactServerApp, { tree: tree, ctx: ctx, preinitScripts: preinitScripts, asNotFound: asNotFound }), ComponentMod, clientReferenceManifest, serverComponentsErrorHandler, serverComponentsPostponeHandler);
        const renderInlinedDataTransformStream = new TransformStream();
        const children = (_jsx(HeadManagerContext.Provider, { value: {
                appDir: true,
                nonce,
            }, children: _jsx(ServerInsertedHTMLProvider, { children: _jsx(ReactServerEntrypoint, { renderReactServer: serverComponentsRenderer, inlinedDataTransformStream: renderInlinedDataTransformStream, clientReferenceManifest: clientReferenceManifest, formState: formState, nonce: nonce }) }) }));
                ...
                onHeaders: (headers) => {
                    ...
                    if (isStaticGeneration) {
                        headers.forEach((value, key) => {
                            var _a;
                            (_a = metadata.headers) !== null && _a !== void 0 ? _a : (metadata.headers = {});
        ...
        try {
            ...
            const options = {
                ...
                // partially rendered.
                validateRootLayout: !postponed && !renderOpts.postponed
                    ? validateRootLayout
                    : undefined,
            ...
            const errorServerComponentsRenderer = createReactServerRenderer(_jsx(ReactServerError, { tree: tree, ctx: ctx, preinitScripts: errorPreinitScripts, errorType: errorType }), ComponentMod, clientReferenceManifest, serverComponentsErrorHandler, serverComponentsPostponeHandler);
            ...
            try {
                const fizzStream = await renderToInitialFizzStream({
                    ...
                    element: (_jsx(ReactServerEntrypoint, { renderReactServer: errorServerComponentsRenderer, inlinedDataTransformStream: errorInlinedDataTransformStream, clientReferenceManifest: clientReferenceManifest, formState: formState, nonce: nonce })),
                    streamOptions: {
                        nonce,
    ...
    if (renderOpts.experimental.ppr &&
        ...
        warn('');
        error(`Prerendering ${urlPathname} needs to partially bail out because something dynamic was used. ` +
    ...
    metadata.revalidate =
        (_c = staticGenerationStore.revalidate) !== null && _c !== void 0 ? _c : ctx.defaultRevalidate;
...
export const renderToHTMLOrFlight = (req, res, pagePath, query, renderOpts) => {
    ...
    return RequestAsyncStorageWrapper.wrap(renderOpts.ComponentMod.requestAsyncStorage, { req, res, renderOpts }, (requestStore) => StaticGenerationAsyncStorageWrapper.wrap(renderOpts.ComponentMod.staticGenerationAsyncStorage, {
