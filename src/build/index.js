...
import { lockfilePatchPromise, teardownTraceSubscriber, teardownHeapProfiler, } from './swc';
...
export function buildCustomRoute(type, route, restrictedRedirectPaths) {
    ...
    if (type !== 'redirect') {
        return Object.assign(Object.assign({}, route), { regex });
    }
    return Object.assign(Object.assign({}, route), { statusCode: getRedirectStatus(route), permanent: undefined, regex });
}
...
async function writeImagesManifest(distDir, config) {
    var _a;
    const images = Object.assign({}, config.images);
    const { deviceSizes, imageSizes } = images;
    images.sizes = [...deviceSizes, ...imageSizes];
    images.remotePatterns = (((_a = config === null || config === void 0 ? void 0 : config.images) === null || _a === void 0 ? void 0 : _a.remotePatterns) || []).map((p) => {
        var _a;
        return ({
            ...
            port: p.port,
            pathname: makeRe((_a = p.pathname) !== null && _a !== void 0 ? _a : '**').source,
        });
    });
...
async function writeStandaloneDirectory(nextBuildSpan, distDir, pageKeys, denormalizedAppPages, outputFileTracingRoot, requiredServerFiles, middlewareManifest, hasInstrumentationHook, staticPages, loadedEnvFiles, appDir) {
    await nextBuildSpan
        ...
        await recursiveCopy(path.join(distDir, SERVER_DIRECTORY, 'pages'), path.join(distDir, STANDALONE_DIRECTORY, path.relative(outputFileTracingRoot, distDir), SERVER_DIRECTORY, 'pages'), { overwrite: true });
        if (appDir) {
            const originalServerApp = path.join(distDir, SERVER_DIRECTORY, 'app');
            if (existsSync(originalServerApp)) {
                await recursiveCopy(originalServerApp, path.join(distDir, STANDALONE_DIRECTORY, path.relative(outputFileTracingRoot, distDir), SERVER_DIRECTORY, 'app'), { overwrite: true });
            }
        }
    });
}
function getNumberOfWorkers(config) {
    ...
    if (config.experimental.memoryBasedWorkersCount) {
        return Math.max(Math.min(config.experimental.cpus || 1, Math.floor(os.freemem() / 1e9)), 
...
function createStaticWorker(config, incrementalCacheIpcPort, incrementalCacheIpcValidationKey) {
    ...
    return new Worker(staticWorkerPath, {
        ...
        forkOptions: {
            env: Object.assign(Object.assign({}, process.env), { __NEXT_INCREMENTAL_CACHE_IPC_PORT: incrementalCacheIpcPort
                    ? incrementalCacheIpcPort + ''
                    : undefined, __NEXT_INCREMENTAL_CACHE_IPC_KEY: incrementalCacheIpcValidationKey }),
        },
...
async function writeFullyStaticExport(config, incrementalCacheIpcPort, incrementalCacheIpcValidationKey, dir, enabledDirectories, configOutDir, nextBuildSpan) {
    ...
    await exportApp(dir, {
        ...
        // exposed methods.
        exportAppPageWorker: appWorker === null || appWorker === void 0 ? void 0 : appWorker.exportPage,
        exportPageWorker: pagesWorker === null || pagesWorker === void 0 ? void 0 : pagesWorker.exportPage,
        endWorker: async () => {
...
export default async function build(dir, reactProductionProfiling = false, debugOutput = false, runLint = true, noMangling = false, appDirOnly = false, turboNextBuild = false, buildMode) {
    ...
    try {
        ...
        await nextBuildSpan.traceAsyncFn(async () => {
            ...
            const customRoutes = await nextBuildSpan
            ...
            const pagesPaths = !appDirOnly && pagesDir
                ? await nextBuildSpan.traceChild('collect-pages').traceAsyncFn(() => recursiveReadDir(pagesDir, {
            ...
            if (appDir) {
                const appPaths = await nextBuildSpan
                    ...
                    .traceAsyncFn(() => recursiveReadDir(appDir, {
                ...
                for (const [pageKey, pagePath] of Object.entries(mappedAppPages)) {
                    if (pageKey.includes('[[...__metadata_id__]]')) {
                        ...
                        if (pageKey.includes('sitemap.xml/[[...__metadata_id__]]') &&
                            isDynamic) {
                            delete mappedAppPages[pageKey];
            ...
            const hasPages404 = (_a = mappedPages['/404']) === null || _a === void 0 ? void 0 : _a.startsWith(PAGES_DIR_ALIAS);
            const hasApp404 = !!(mappedAppPages === null || mappedAppPages === void 0 ? void 0 : mappedAppPages['/_not-found']);
            ...
            const routesManifest = nextBuildSpan
                .traceChild('generate-routes-manifest')
                .traceFn(() => {
                var _a;
                const sortedRoutes = getSortedRoutes([
                    ...pageKeys.pages,
                    ...((_a = pageKeys.app) !== null && _a !== void 0 ? _a : []),
                ]);
                const dynamicRoutes = [];
                const staticRoutes = [];
            ...
            if (config.experimental.clientRouterFilter) {
                ...
                const clientRouterFilters = createClientRouterFilter(appPaths, config.experimental.clientRouterFilterRedirects
            ...
            const requiredServerFilesManifest = nextBuildSpan
                ...
                const serverFilesManifest = {
                    version: 1,
                    config: Object.assign(Object.assign(Object.assign(Object.assign({}, config), { configFile: undefined }), (ciEnvironment.hasNextSupport
                        ? {
                            compress: false,
                        }
                        : {})), { cacheHandler: cacheHandler
                            ? path.relative(distDir, cacheHandler)
                            : config.cacheHandler, experimental: Object.assign(Object.assign({}, config.experimental), { trustHostHeader: ciEnvironment.hasNextSupport, 
            ...
            const collectServerBuildTracesInParallel = config.experimental.parallelServerBuildTraces ||
            ...
            if (!isGenerateMode) {
                if (runServerAndEdgeInParallel || collectServerBuildTracesInParallel) {
                    ...
                    const _d = turboNextBuild
                        ? await turbopackBuild()
                        : await webpackBuild(useBuildWorker, null), { duration: webpackBuildDuration } = _d, rest = __rest(_d, ["duration"]);
            ...
            if (config.experimental.staticWorkerRequestDeduping) {
                ...
                const cacheInitialization = await initializeIncrementalCache({
                    ...
                    getPrerenderManifest: () => ({
                        version: -1, // letting us know this doesn't conform to spec
                        routes: {},
                        dynamicRoutes: {},
                        notFoundRoutes: [],
            ...
            const appStaticWorkers = appDir
                ? createStaticWorker(config, incrementalCacheIpcPort, incrementalCacheIpcValidationKey)
                : undefined;
            ...
            const { customAppGetInitialProps, namedExports, isNextImageImported, hasSsrAmpPages, hasNonStaticErrorPage, } = await staticCheckSpan.traceAsyncFn(async () => {
                ...
                const errorPageHasCustomGetInitialProps = nonStaticErrorPageSpan.traceAsyncFn(async () => hasCustomErrorPage &&
                    (await pagesStaticWorkers.hasCustomGetInitialProps('/_error', distDir, runtimeEnvConfig, false)));
                const errorPageStaticResult = nonStaticErrorPageSpan.traceAsyncFn(async () => {
                    ...
                    return hasCustomErrorPage &&
                        pagesStaticWorkers.isPageStatic({
                            ...
                            locales: (_a = config.i18n) === null || _a === void 0 ? void 0 : _a.locales,
                            defaultLocale: (_b = config.i18n) === null || _b === void 0 ? void 0 : _b.defaultLocale,
                ...
                const computedManifestData = await computeFromManifest({ build: buildManifest, app: appBuildManifest }, distDir, config.experimental.gzipSize);
                const middlewareManifest = require(path.join(distDir, SERVER_DIRECTORY, MIDDLEWARE_MANIFEST));
                const actionManifest = appDir
                    ? require(path.join(distDir, SERVER_DIRECTORY, SERVER_REFERENCE_MANIFEST + '.json'))
                    : null;
                ...
                for (const key of Object.keys(middlewareManifest === null || middlewareManifest === void 0 ? void 0 : middlewareManifest.functions)) {
                    if (key.startsWith('/api')) {
                        edgeRuntimePagesCount++;
                    }
                }
                await Promise.all(Object.entries(pageKeys)
                    ...
                    return checkPageSpan.traceAsyncFn(async () => {
                        ...
                        if (pageType === 'pages') {
                            pagePath =
                                pagesPaths.find((p) => {
                                    ...
                                    return (p.startsWith(actualPage + '.') ||
                        ...
                        const pageFilePath = isAppBuiltinNotFoundPage(pagePath)
                            ? require.resolve('next/dist/client/components/not-found-error')
                        ...
                        const staticInfo = pagePath
                            ...
                            : undefined;
                        if (staticInfo === null || staticInfo === void 0 ? void 0 : staticInfo.extraConfig) {
                        ...
                        const pageRuntime = middlewareManifest.functions[originalAppPath || page]
                            ? 'edge'
                            : staticInfo === null || staticInfo === void 0 ? void 0 : staticInfo.runtime;
                        if (!isCompileMode) {
                            isServerComponent =
                                pageType === 'app' &&
                                    (staticInfo === null || staticInfo === void 0 ? void 0 : staticInfo.rsc) !== RSC_MODULE_TYPES.client;
                                ...
                                try {
                                    ...
                                    if (pageType === 'app' && originalAppPath) {
                                        ...
                                        if (isEdgeRuntime(pageRuntime)) {
                                            ...
                                            if (workerResult.encodedPrerenderRoutes &&
                                            ...
                                            if (appConfig.revalidate !== 0) {
                                                ...
                                                const hasGenerateStaticParams = !!((_a = workerResult.prerenderRoutes) === null || _a === void 0 ? void 0 : _a.length);
                                        ...
                                        if (workerResult.isStatic === false &&
                                        ...
                                        if (hasPages404 && page === '/404') {
                                            if (!workerResult.isStatic &&
                                                !workerResult.hasStaticProps) {
                                                throw new Error(`\`pages/404\` ${STATIC_STATUS_PAGE_GET_INITIAL_PROPS_ERROR}`);
                                            }
                                            // we need to ensure the 404 lambda is present since we use
                                            // it when _app has getInitialProps
                                            if ((await customAppGetInitialPropsPromise) &&
                                                !workerResult.hasStaticProps) {
                                                staticPages.delete(page);
                                            }
                                        }
                                        if (STATIC_STATUS_PAGES.includes(page) &&
                ...
                const nonStaticErrorPage = (await errorPageHasCustomGetInitialProps) ||
            ...
            if (customAppGetInitialProps) {
                console.warn(bold(yellow(`Warning: `)) +
                    yellow(`You have opted-out of Automatic Static Optimization due to \`getInitialProps\` in \`pages/_app\`. This does not opt-out pages with \`getStaticProps\``));
            ...
            const { i18n } = config;
            const usedStaticStatusPages = STATIC_STATUS_PAGES.filter((page) => mappedPages[page] &&
            ...
            if (!isCompileMode &&
                ...
                await staticGenerationSpan.traceAsyncFn(async () => {
                    ...
                    const exportApp = require('../export')
                        .default;
                    const exportConfig = Object.assign(Object.assign({}, config), { 
                        ...
                        exportPathMap: (defaultMap) => {
                            ...
                            additionalSsgPaths.forEach((routes, page) => {
                                ...
                                routes.forEach((route, routeIdx) => {
                                    defaultMap[route] = {
                                        page,
                                        query: { __nextSsgPath: encodedRoutes === null || encodedRoutes === void 0 ? void 0 : encodedRoutes[routeIdx] },
                                    };
                                });
                            });
                            ...
                            if (i18n) {
                                for (const page of [
                                    ...
                                    for (const locale of i18n.locales) {
                                        ...
                                        if (isSsg && isDynamic && !isFallback)
                                            continue;
                                        const outputPath = `/${locale}${page === '/' ? '' : page}`;
                                        defaultMap[outputPath] = {
                                            page: ((_a = defaultMap[page]) === null || _a === void 0 ? void 0 : _a.page) || page,
                                            query: {
                                                __nextLocale: locale,
                    ...
                    const exportOptions = {
                        ...
                        // The worker already explicitly binds `this` to each of the
                        // exposed methods.
                        exportAppPageWorker: appStaticWorkers === null || appStaticWorkers === void 0 ? void 0 : appStaticWorkers.exportPage,
                        exportPageWorker: pagesStaticWorkers === null || pagesStaticWorkers === void 0 ? void 0 : pagesStaticWorkers.exportPage,
                        endWorker: async () => {
                            await pagesStaticWorkers.end();
                            await (appStaticWorkers === null || appStaticWorkers === void 0 ? void 0 : appStaticWorkers.end());
                        },
                    };
                    ...
                    for (const [originalAppPath, routes] of appStaticPaths) {
                        ...
                        let hasDynamicData = appConfig.revalidate === 0 ||
                            ((_a = exportResult.byPath.get(page)) === null || _a === void 0 ? void 0 : _a.revalidate) === 0;
                        if (hasDynamicData && ((_b = pageInfos.get(page)) === null || _b === void 0 ? void 0 : _b.isStatic)) {
                            // if the page was marked as being static, but it contains dynamic data
                            // (ie, in the case of a static generation bailout), then it should be marked dynamic
                            pageInfos.set(page, Object.assign(Object.assign({}, pageInfos.get(page)), { isStatic: false, isSSG: false }));
                        }
                        const isRouteHandler = isAppRouteRoute(originalAppPath);
                        // When this is an app page and PPR is enabled, the route supports
                        // partial pre-rendering.
                        const experimentalPPR = !isRouteHandler && config.experimental.ppr === true
                            ? true
                            : undefined;
                        ...
                        routes.forEach((route) => {
                            var _a, _b;
                            if (isDynamicRoute(page) && route === page)
                                return;
                            if (route === '/_not-found')
                                return;
                            const { revalidate = (_a = appConfig.revalidate) !== null && _a !== void 0 ? _a : false, metadata = {}, hasEmptyPrelude, hasPostponed, } = (_b = exportResult.byPath.get(route)) !== null && _b !== void 0 ? _b : {};
                            pageInfos.set(route, Object.assign(Object.assign({}, pageInfos.get(route)), { hasPostponed,
                                hasEmptyPrelude }));
                            // update the page (eg /blog/[slug]) to also have the postpone metadata
                            pageInfos.set(page, Object.assign(Object.assign({}, pageInfos.get(page)), { hasPostponed,
                                hasEmptyPrelude }));
                            if (revalidate !== 0) {
                                ...
                                finalPrerenderRoutes[route] = Object.assign(Object.assign({}, routeMeta), { experimentalPPR, experimentalBypassFor: bypassFor, initialRevalidateSeconds: revalidate, srcRoute: page, dataRoute,
                                    prefetchDataRoute });
                            }
                            else {
                                hasDynamicData = true;
                                ...
                                // used dynamic data
                                pageInfos.set(route, Object.assign(Object.assign({}, pageInfos.get(route)), { isSSG: false, isStatic: false }));
                            }
                        });
                        if (!hasDynamicData && isDynamicRoute(originalAppPath)) {
                            ...
                            pageInfos.set(page, Object.assign(Object.assign({}, pageInfos.get(page)), { isDynamicAppRoute: true, 
                            ...
                            finalDynamicRoutes[page] = {
                                ...
                                dataRouteRegex: isRouteHandler
                                    ? null
                                    : normalizeRouteRegex(getNamedRouteRegex(dataRoute.replace(/\.rsc$/, ''), false).re.source.replace(/\(\?:\\\/\)\?\$$/, '\\.rsc$')),
                                prefetchDataRoute,
                                prefetchDataRouteRegex: isRouteHandler || !prefetchDataRoute
                                    ? undefined
                                    : normalizeRouteRegex(getNamedRouteRegex(prefetchDataRoute.replace(/\.prefetch\.rsc$/, ''), false).re.source.replace(/\(\?:\\\/\)\?\$$/, '\\.prefetch\\.rsc$')),
                            };
                        }
                    }
                    const moveExportedPage = async (originPage, page, file, isSsg, ext, additionalSsgFile = false) => {
                        return staticGenerationSpan
                            ...
                            const relativeDest = path
                                .relative(path.join(distDir, SERVER_DIRECTORY), path.join(path.join(pagePath, 
                            ...
                            if (!isDynamic) {
                                ...
                                if (i18n) {
                                    ...
                                    for (const locale of i18n.locales) {
                                        ...
                                        finalPrerenderRoutes[localePage] = {
                                            initialRevalidateSeconds: (_d = (_c = exportResult.byPath.get(localePage)) === null || _c === void 0 ? void 0 : _c.revalidate) !== null && _d !== void 0 ? _d : false,
                                    ...
                                    finalPrerenderRoutes[page] = {
                                        initialRevalidateSeconds: (_f = (_e = exportResult.byPath.get(page)) === null || _e === void 0 ? void 0 : _e.revalidate) !== null && _f !== void 0 ? _f : false,
                                        experimentalPPR: undefined,
                                        srcRoute: null,
                                        dataRoute: path.posix.join('/_next/data', buildId, `${file}.json`),
                                ...
                                if (pageInfo) {
                                    pageInfo.initialRevalidateSeconds =
                                        (_h = (_g = exportResult.byPath.get(page)) === null || _g === void 0 ? void 0 : _g.revalidate) !== null && _h !== void 0 ? _h : false;
                                }
                            }
                            else {
                                ...
                                for (const route of extraRoutes) {
                                    ...
                                    const initialRevalidateSeconds = (_k = (_j = exportResult.byPath.get(route)) === null || _j === void 0 ? void 0 : _j.revalidate) !== null && _k !== void 0 ? _k : false;
            ...
            appStaticWorkers === null || appStaticWorkers === void 0 ? void 0 : appStaticWorkers.close();
            ...
            telemetry.record(eventBuildOptimize(pagesPaths, {
                ...
                ssrPageCount: pagesPaths.length -
                    (staticPages.size + ssgPages.size + serverPropsPages.size),
                hasStatic404: useStaticPages404,
                hasReportWebVitals: (_b = namedExports === null || namedExports === void 0 ? void 0 : namedExports.includes('reportWebVitals')) !== null && _b !== void 0 ? _b : false,
            ...
            if (ssgPages.size > 0 || appDir) {
                ...
                await writeClientSsgManifest(prerenderManifest, {
                    distDir,
                    buildId,
                    locales: ((_c = config.i18n) === null || _c === void 0 ? void 0 : _c.locales) || [],
                });
            }
            else {
            ...
            if (config.analyticsId) {
                console.log(bold(green('Next.js Speed Insights')) +
