...
export async function loadEntrypoint(entrypoint, replacements, injections, imports) {
    ...
    file = file.replaceAll(/from "(\..*)"|import "(\..*)"/g, function (_, fromRequest, importRequest) {
        count++;
        const relative = path
            .relative(PACKAGE_ROOT, path.resolve(TEMPLATE_FOLDER, fromRequest !== null && fromRequest !== void 0 ? fromRequest : importRequest))
    ...
    file = file.replaceAll(new RegExp(`${Object.keys(replacements)
        .map((k) => `"${k}"`)
        .join('|')}`, 'g'), (match) => {
    ...
    if (injected.size !== Object.keys(injections !== null && injections !== void 0 ? injections : {}).length) {
        ...
        const difference = Object.keys(injections !== null && injections !== void 0 ? injections : {}).filter((key) => !injected.has(key));
    ...
    if (importsAdded.size !== Object.keys(imports !== null && imports !== void 0 ? imports : {}).length) {
        ...
        // not used but were provided.
        const difference = Object.keys(imports !== null && imports !== void 0 ? imports : {}).filter((key) => !importsAdded.has(key));
