...
export function normalizeRewritesForBuildManifest(rewrites) {
    var _a, _b, _c;
    return {
        afterFiles: (_a = rewrites.afterFiles) === null || _a === void 0 ? void 0 : _a.map((item) => normalizeRewrite(item)),
        beforeFiles: (_b = rewrites.beforeFiles) === null || _b === void 0 ? void 0 : _b.map((item) => normalizeRewrite(item)),
        fallback: (_c = rewrites.fallback) === null || _c === void 0 ? void 0 : _c.map((item) => normalizeRewrite(item)),
    };
}
...
export function getEntrypointFiles(entrypoint) {
    var _a;
    return ((_a = entrypoint === null || entrypoint === void 0 ? void 0 : entrypoint.getFiles().filter((file) => {
        // We don't want to include `.hot-update.js` files into the initial page
        return /(?<!\.hot-update)\.(js|css)($|\?)/.test(file);
    }).map((file) => file.replace(/\\/g, '/'))) !== null && _a !== void 0 ? _a : []);
}
const processRoute = (r) => {
    const rewrite = Object.assign({}, r);
        ...
        return createAssetsSpan === null || createAssetsSpan === void 0 ? void 0 : createAssetsSpan.traceFn(() => {
            ...
            if (this.exportRuntime) {
                assets[`server/${MIDDLEWARE_BUILD_MANIFEST}.js`] =
                    new sources.RawSource(`self.__BUILD_MANIFEST=${JSON.stringify(assetMap)}`);
            }
            if (!this.isDevFallback) {
