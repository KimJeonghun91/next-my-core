...
import { traverseModules, forEachEntryModule, formatBarrelOptimizedResource, } from '../utils';
...
function deduplicateCSSImportsForEntry(mergedCSSimports) {
    ...
    for (const [entryName, cssImports] of sortedCSSImports) {
        for (const cssImport of cssImports) {
            if (trackedCSSImports.has(cssImport))
                continue;
...
export class FlightClientEntryPlugin {
    ...
    apply(compiler) {
        ...
        compiler.hooks.afterCompile.tap(PLUGIN_NAME, (compilation) => {
            const recordModule = (modId, mod) => {
                ...
                const modPath = mod.matchResource || ((_a = mod.resourceResolveData) === null || _a === void 0 ? void 0 : _a.path);
                const modQuery = ((_b = mod.resourceResolveData) === null || _b === void 0 ? void 0 : _b.query) || '';
    ...
    async createClientEntries(compiler, compilation) {
        ...
        forEachEntryModule(compilation, ({ name, entryModule }) => {
            ...
            for (const clientEntryToInject of clientEntriesToInject) {
                const injected = this.injectClientEntryAndSSRModules(Object.assign(Object.assign({}, clientEntryToInject), { clientImports: [
    ...
    collectClientActionsFromDependencies({ compilation, dependencies, }) {
        ...
        const collectActions = ({ entryRequest, resolvedModule, }) => {
            const collectActionsInDep = (mod) => {
                ...
                let modRequest = ((_a = mod.resourceResolveData) === null || _a === void 0 ? void 0 : _a.path) + ((_b = mod.resourceResolveData) === null || _b === void 0 ? void 0 : _b.query);
                ...
                if ((_c = mod.matchResource) === null || _c === void 0 ? void 0 : _c.startsWith(BARREL_OPTIMIZATION_PREFIX)) {
                    modRequest = mod.matchResource + ':' + modRequest;
                }
                if (!modRequest || visitedModule.has(modRequest))
                    return;
    ...
    collectComponentInfoFromServerEntryDependency({ entryRequest, compilation, resolvedModule, }) {
        ...
        const filterClientComponents = (mod) => {
            ...
            if (!modRequest || visited.has(modRequest))
                return;
            ...
            if (isCSS) {
                ...
                if (sideEffectFree) {
                    const unused = !compilation.moduleGraph
                        .getExportsInfo(mod)
                        .isModuleUsed(this.isEdgeServer ? EDGE_RUNTIME_WEBPACK : 'webpack-runtime');
                    if (unused)
                        return;
                }
    ...
    injectClientEntryAndSSRModules({ compiler, compilation, entryName, clientImports, bundlePath, absolutePagePath, }) {
        ...
        const clientLoader = `next-flight-client-entry-loader?${stringify({
            modules: this.isEdgeServer
                ? loaderOptions.modules.map((importPath) => importPath.replace(/[\\/]next[\\/]dist[\\/]esm[\\/]/, '/next/dist/'.replace(/\//g, path.sep)))
                : loaderOptions.modules,
            server: false,
        })}!`;
        const clientSSRLoader = `next-flight-client-entry-loader?${stringify(Object.assign(Object.assign({}, loaderOptions), { server: true }))}!`;
    ...
    addEntry(compilation, context, dependency, options) {
    ...
    async createActionAssets(compilation, assets) {
        ...
        for (let id in pluginState.serverActions) {
            ...
            for (let name in action.workers) {
                const modId = pluginState.actionModServerId[name][action.layer[name] === WEBPACK_LAYERS.actionBrowser
                    ? 'client'
                    : 'server'];
        ...
        for (let id in pluginState.edgeServerActions) {
            ...
            for (let name in action.workers) {
                const modId = pluginState.actionModEdgeServerId[name][action.layer[name] === WEBPACK_LAYERS.actionBrowser
                    ? 'client'
                    : 'server'];
                action.workers[name] = modId;
            }
        ...
        assets[`${this.assetPrefix}${SERVER_REFERENCE_MANIFEST}.js`] =
            new sources.RawSource(`self.__RSC_SERVER_MANIFEST=${JSON.stringify(json)}`);
