...
import { resolveRequests, normalizeUrl, requestify, isUrlRequestable, isDataUrl, 
...
function parseDeclaration(declaration, key, result, isSupportDataURLInNewURL) {
    ...
    const parsed = valueParser(declaration.raws && declaration.raws.value && declaration.raws.value.raw
    ...
    parsed.walk((valueNode, index, valueNodes) => {
        ...
        if (isUrlFunc.test(valueNode.value)) {
            ...
            if ((isIgnoreOnDeclaration && typeof needIgnore === 'undefined') ||
                needIgnore) {
                if (needIgnore) {
            ...
            return {
                ...
                async OnceExit() {
                    ...
                    const resolvedDeclarations = await Promise.all(parsedDeclarations.map(async (parsedDeclaration) => {
                        ...
                        const request = requestify(pathname, rootContext, 
                        // @ts-expect-error TODO: only 2 arguments allowed.
                        needToResolveURL);
                        if (!needToResolveURL) {
                            // eslint-disable-next-line consistent-return
                            return Object.assign(Object.assign({}, parsedDeclaration), { url: request, hash });
                        }
                        ...
                        // eslint-disable-next-line consistent-return
                        return Object.assign(Object.assign({}, parsedDeclaration), { url: resolvedUrl, hash });
                    }));
