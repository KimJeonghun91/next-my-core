...
async function createTreeCodeFromPath(pagePath, { page, resolveDir, resolver, resolveParallelSegments, metadataResolver, pageExtensions, basePath, }) {
    ...
    async function resolveAdjacentParallelSegments(segmentPath) {
        ...
        const parallelSegments = ['children'];
        try {
    ...
    async function createSubtreePropsFromSegmentPath(segments) {
        ...
        for (const [parallelKey, parallelSegment] of parallelSegments) {
          ...
          ${createMetadataExportsCode(metadata)}
            ...
            const isFirstLayerGroupRoute = segments.length === 1 &&
            ...
            if (!rootLayout) {
                const layoutPath = (_a = definedFilePaths.find(([type]) => type === 'layout')) === null || _a === void 0 ? void 0 : _a[1];
            ...
            if (isNotFoundRoute && normalizedParallelKey === 'children') {
                const notFoundPath = (_c = (_b = definedFilePaths.find(([type]) => type === 'not-found')) === null || _b === void 0 ? void 0 : _b[1]) !== null && _c !== void 0 ? _c : defaultNotFoundPath;
                subtreeCode = `{
        ...
        for (const adjacentParallelSegment of adjacentParallelSegments) {
            if (!props[normalizeParallelKey(adjacentParallelSegment)]) {
                ...
                if (!defaultPath) {
                    ...
                    defaultPath = normalizedDefault !== null && normalizedDefault !== void 0 ? normalizedDefault : PARALLEL_ROUTE_DEFAULT_PATH;
                }
    ...
    return {
        ...
        rootLayout,
        globalError: globalError !== null && globalError !== void 0 ? globalError : defaultGlobalErrorPath,
    };
}
function createAbsolutePath(appDir, pathToTurnAbsolute) {
    return (pathToTurnAbsolute
...
const nextAppLoader = async function nextAppLoader() {
    ...
    const normalizedAppPaths = typeof appPaths === 'string' ? [appPaths] : appPaths || [];
    ...
    if (!treeCodeResult.rootLayout) {
        if (!isDev) {
            ...
            if (!createdRootLayout) {
                ...
                if (rootLayoutPath) {
                    message += `We tried to create ${bold(path.relative((_b = (_a = this._compiler) === null || _a === void 0 ? void 0 : _a.context) !== null && _b !== void 0 ? _b : '', rootLayoutPath))} for you but something went wrong.`;
                }
                else {
                    message +=
