...
const edgeSSRLoader = async function edgeSSRLoader() {
    ...
    const userland500Path = absolute500Path
        ? this.utils.contextify(this.context || this.rootContext, swapDistFolderWithEsmDistFolder(absolute500Path))
        : null;
    ...
    if (isAppDir) {
        return await loadEntrypoint('edge-ssr-app', {
            ...
            serverActions: typeof serverActions === 'undefined'
                ? 'undefined'
                : JSON.stringify(serverActions),
        }, {
            incrementalCacheHandler: cacheHandler !== null && cacheHandler !== void 0 ? cacheHandler : null,
        });
    }
    else {
