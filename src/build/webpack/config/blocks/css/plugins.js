...
function isIgnoredPlugin(pluginPath) {
    ...
    console.warn(`${yellow(bold('Warning'))}: Please remove the ${underline(plugin)} plugin from your PostCSS configuration. ` +
    ...
    return [
        ...
        disablePostcssPresetEnv
            ...
            : [
                ...
                {
                    browsers: supportedBrowsers !== null && supportedBrowsers !== void 0 ? supportedBrowsers : ['defaults'],
                    autoprefixer: {
...
export async function getPostCssPlugins(dir, supportedBrowsers, disablePostcssPresetEnv = false) {
    ...
    if (invalidKey) {
        console.warn(`${yellow(bold('Warning'))}: Your PostCSS configuration defines a field which is not supported (\`${invalidKey}\`). ` +
