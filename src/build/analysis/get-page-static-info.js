...
export function getRSCModuleInformation(source, isReactServerLayer) {
    ...
    const actions = actionsJson
        ? Object.values(JSON.parse(actionsJson[1]))
        : undefined;
    ...
    const clientRefs = (_a = clientInfoMatch === null || clientInfoMatch === void 0 ? void 0 : clientInfoMatch[1]) === null || _a === void 0 ? void 0 : _a.split(',');
    const clientEntryType = clientInfoMatch === null || clientInfoMatch === void 0 ? void 0 : clientInfoMatch[2];
...
function warnInvalidValue(pageFilePath, key, message) {
    if (warnedInvalidValueMap[key].has(pageFilePath))
        return;
        ...
        try {
            ...
            for (const node of swcAST.body) {
                ...
                if (node.type === 'ExportDeclaration' &&
                    ...
                    for (const declaration of (_b = node.declaration) === null || _b === void 0 ? void 0 : _b.declarations) {
                ...
                if (node.type === 'ExportDeclaration' &&
                    ((_c = node.declaration) === null || _c === void 0 ? void 0 : _c.type) === 'FunctionDeclaration' &&
                    exportsSet.has((_d = node.declaration.identifier) === null || _d === void 0 ? void 0 : _d.value)) {
                ...
                if (node.type === 'ExportDeclaration' &&
                    ((_e = node.declaration) === null || _e === void 0 ? void 0 : _e.type) === 'VariableDeclaration') {
                    const id = (_g = (_f = node.declaration) === null || _f === void 0 ? void 0 : _f.declarations[0]) === null || _g === void 0 ? void 0 : _g.id.value;
                ...
                if (node.type === 'ExportNamedDeclaration') {
                    const values = node.specifiers.map((specifier) => {
                        ...
                        return specifier.type === 'ExportSpecifier' &&
                            ((_a = specifier.orig) === null || _a === void 0 ? void 0 : _a.type) === 'Identifier' &&
                            ((_b = specifier.orig) === null || _b === void 0 ? void 0 : _b.value);
                    });
                    for (const value of values) {
                        if (!ssg && value === 'getStaticProps')
                            ssg = true;
                        if (!ssr && value === 'getServerSideProps')
                            ssr = true;
                        ...
                        if (!runtime && value === 'runtime')
                            warnInvalidValue(pageFilePath, 'runtime', 'it was not assigned to a string literal');
                        if (!preferredRegion && value === 'preferredRegion')
                            warnInvalidValue(pageFilePath, 'preferredRegion', 'it was not assigned to a string literal or an array of string literals');
                    }
                }
            }
            return {
...
export function getMiddlewareMatchers(matcherOrMatchers, nextConfig) {
    ...
    let routes = matchers.map((m) => {
        let middleware = (typeof m === 'string' ? { source: m } : m);
        if (middleware) {
    ...
    routes = routes.map((r) => {
        let { source } = r;
        const isRoot = source === '/';
        if ((i18n === null || i18n === void 0 ? void 0 : i18n.locales) && r.locale !== false) {
            source = `/:nextInternalLocale((?!_next/)[^/.]{1,})${isRoot ? '' : source}`;
        }
        source = `/:nextData(_next/data/[^/]{1,})?${source}${isRoot
    ...
    checkCustomRoutes(routes, 'middleware');
    return routes.map((r) => {
        const { source } = r, rest = __rest(r, ["source"]);
        const parsedPage = tryToParsePath(source);
        if (parsedPage.error || !parsedPage.regexStr) {
            throw new Error(`Invalid source: ${source}`);
        }
        const originalSource = originalSourceMap.get(r);
        return Object.assign(Object.assign({}, rest), { regexp: parsedPage.regexStr, originalSource: originalSource || source });
    });
}
function getMiddlewareConfig(pageFilePath, config, nextConfig) {
    ...
    if (config.unstable_allowDynamic) {
        result.unstable_allowDynamicGlobs = Array.isArray(config.unstable_allowDynamic)
            ? config.unstable_allowDynamic
            : [config.unstable_allowDynamic];
        for (const glob of (_a = result.unstable_allowDynamicGlobs) !== null && _a !== void 0 ? _a : []) {
            try {
                matcher(glob);
            }
            catch (err) {
...
const warnedUnsupportedValueMap = new LRUCache({ max: 250 });
function warnAboutUnsupportedValue(pageFilePath, page, error) {
    if (warnedUnsupportedValueMap.has(pageFilePath)) {
        return;
    }
    Log.warn(`Next.js can't recognize the exported \`config\` field in ` +
    ...
    if (/(?<!(_jsx|jsx-))runtime|preferredRegion|getStaticProps|getServerSideProps|generateStaticParams|export const/.test(fileContent)) {
        ...
        if (extraProperties && pageType === PAGE_TYPES.APP) {
            for (const prop of extraProperties) {
                if (!AUTHORIZED_EXTRA_ROUTER_PROPS.includes(prop))
                    continue;
                try {
            ...
            for (const key in config) {
                if (!AUTHORIZED_EXTRA_ROUTER_PROPS.includes(key))
                    continue;
        ...
        const isAnAPIRoute = isAPIRoute(page === null || page === void 0 ? void 0 : page.replace(/^(?:\/src)?\/pages\//, '/'));
        resolvedRuntime =
        ...
        const middlewareConfig = getMiddlewareConfig(page !== null && page !== void 0 ? page : 'middleware/edge API route', config, nextConfig);
        if (pageType === PAGE_TYPES.APP &&
            (directives === null || directives === void 0 ? void 0 : directives.has('client')) &&
            generateStaticParams) {
            throw new Error(`Page "${page}" cannot use both "use client" and export function "generateStaticParams()".`);
        }
        return Object.assign(Object.assign(Object.assign({ ssr,
            ssg,
            rsc,
            generateStaticParams, amp: config.amp || false }, (middlewareConfig && { middleware: middlewareConfig })), (resolvedRuntime && { runtime: resolvedRuntime })), { preferredRegion,
            extraConfig });
    }
    return {
        ssr: false,
