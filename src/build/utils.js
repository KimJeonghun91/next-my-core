...
export async function computeFromManifest(manifests, distPath, gzipSize = true, pageInfos) {
    var _a, _b, _c;
    if (Object.is(cachedBuildManifest, manifests.build) &&
    ...
    if ((_a = manifests.app) === null || _a === void 0 ? void 0 : _a.pages) {
        files.app = { each: new Map(), expected: 0 };
    ...
    await Promise.all([
        ...new Set([
            ...
            ...((_c = (_b = files.app) === null || _b === void 0 ? void 0 : _b.each.keys()) !== null && _c !== void 0 ? _c : []),
        ]),
    ].map(async (f) => {
        try {
...
export function isInstrumentationHookFilename(file) {
    return (file === INSTRUMENTATION_HOOK_FILENAME ||
...
export async function printTreeView(lists, pageInfos, { distPath, buildId, pagesDir, pageExtensions, buildManifest, appBuildManifest, middlewareManifest, useStaticPages404, gzipSize = true, }) {
    ...
    const getCleanName = (fileName) => fileName
        // Trim off `static/`
        .replace(/^static\//, '')
    ...
    const messages = [];
    const stats = await computeFromManifest({ build: buildManifest, app: appBuildManifest }, distPath, gzipSize, pageInfos);
    const printFileTree = async ({ list, routerType, }) => {
        ...
        filteredPages.forEach((item, i, arr) => {
            ...
            const totalDuration = ((pageInfo === null || pageInfo === void 0 ? void 0 : pageInfo.pageDuration) || 0) +
                (((_a = pageInfo === null || pageInfo === void 0 ? void 0 : pageInfo.ssgPageDurations) === null || _a === void 0 ? void 0 : _a.reduce((a, b) => a + (b || 0), 0)) || 0);
            let symbol;
            if (item === '/_app' || item === '/_app.server') {
                symbol = ' ';
            }
            else if (isEdgeRuntime(pageInfo === null || pageInfo === void 0 ? void 0 : pageInfo.runtime)) {
                symbol = 'ℇ';
            }
            else if (pageInfo === null || pageInfo === void 0 ? void 0 : pageInfo.isPPR) {
                if (
                // If the page has an empty prelude, then it's equivalent to a dynamic page
                (pageInfo === null || pageInfo === void 0 ? void 0 : pageInfo.hasEmptyPrelude) ||
                ...
                else if (!(pageInfo === null || pageInfo === void 0 ? void 0 : pageInfo.hasPostponed)) {
                    symbol = '○';
                }
                else {
                    symbol = '◐';
            ...
            else if (pageInfo === null || pageInfo === void 0 ? void 0 : pageInfo.isStatic) {
                symbol = '○';
            }
            else if (pageInfo === null || pageInfo === void 0 ? void 0 : pageInfo.isSSG) {
                symbol = '●';
            }
            else {
                symbol = 'λ';
            ...
            if (pageInfo === null || pageInfo === void 0 ? void 0 : pageInfo.initialRevalidateSeconds)
                usedSymbols.add('ISR');
            messages.push([
                `${border} ${symbol} ${(pageInfo === null || pageInfo === void 0 ? void 0 : pageInfo.initialRevalidateSeconds)
                    ? `${item} (ISR: ${pageInfo === null || pageInfo === void 0 ? void 0 : pageInfo.initialRevalidateSeconds} Seconds)`
                    : item}${totalDuration > MIN_DURATION
            ...
            const uniqueCssFiles = ((_b = buildManifest.pages[item]) === null || _b === void 0 ? void 0 : _b.filter((file) => {
                var _a;
                return file.endsWith('.css') &&
                    ((_a = stats.router[routerType]) === null || _a === void 0 ? void 0 : _a.unique.files.includes(file));
            })) || [];
            ...
            if ((_c = pageInfo === null || pageInfo === void 0 ? void 0 : pageInfo.ssgPageRoutes) === null || _c === void 0 ? void 0 : _c.length) {
                const totalRoutes = pageInfo.ssgPageRoutes.length;
                const contSymbol = i === arr.length - 1 ? ' ' : '├';
                let routes;
                if (pageInfo.ssgPageDurations &&
                ...
                routes.forEach(({ route, duration, avgDuration }, index, { length }) => {
                    const innerSymbol = index === length - 1 ? '└' : '├';
                    messages.push([
                        `${contSymbol}   ${innerSymbol} ${route}${duration > MIN_DURATION
        ...
        const sharedFilesSize = (_a = stats.router[routerType]) === null || _a === void 0 ? void 0 : _a.common.size.total;
        const sharedFiles = (_c = (_b = stats.router[routerType]) === null || _b === void 0 ? void 0 : _b.common.files) !== null && _c !== void 0 ? _c : [];
        messages.push([
    ...
    pageInfos.set('/404', Object.assign(Object.assign({}, (pageInfos.get('/404') || pageInfos.get('/_error'))), { isStatic: useStaticPages404 }));
    // If there's no app /_notFound page present, then the 404 is still using the pages/404
    if (!lists.pages.includes('/404') && !((_a = lists.app) === null || _a === void 0 ? void 0 : _a.includes('/_not-found'))) {
    ...
    const middlewareInfo = (_b = middlewareManifest.middleware) === null || _b === void 0 ? void 0 : _b['/'];
    if ((middlewareInfo === null || middlewareInfo === void 0 ? void 0 : middlewareInfo.files.length) > 0) {
        const middlewareSizes = await Promise.all(middlewareInfo.files
...
export async function getJsPageSizeInKb(routerType, page, distPath, buildManifest, appBuildManifest, gzipSize = true, cachedStats) {
    ...
    const stats = cachedStats !== null && cachedStats !== void 0 ? cachedStats : (await computeFromManifest({ build: buildManifest, app: appBuildManifest }, distPath, gzipSize));
    ...
    const pagePath = routerType === 'pages'
        ? denormalizePagePath(page)
        : denormalizeAppPagePath(page);
    const fnFilterJs = (entry) => entry.endsWith('.js');
    const pageFiles = ((_a = pageManifest.pages[pagePath]) !== null && _a !== void 0 ? _a : []).filter(fnFilterJs);
    const appFiles = ((_b = pageManifest.pages['/_app']) !== null && _b !== void 0 ? _b : []).filter(fnFilterJs);
    ...
    toPrerender.forEach((entry) => {
        ...
        // required keys.
            ...
            if (invalidKeys.length) {
                throw new Error(`Additional keys were returned from \`getStaticPaths\` in page "${page}". ` +
            ...
            _validParamKeys.forEach((validParamKey) => {
                ...
                builtPage = builtPage
                    .replace(replaced, repeat
                    ? paramValue
                        .map((segment) => escapePathDelimiters(segment, true))
                        .join('/')
                    : escapePathDelimiters(paramValue, true))
                    .replace(/\\/g, '/')
                    .replace(/(?!^)\/$/, '');
...
export const collectAppConfig = (mod) => {
    let hasConfig = false;
    const config = {};
    if (typeof (mod === null || mod === void 0 ? void 0 : mod.revalidate) !== 'undefined') {
        config.revalidate = mod.revalidate;
        hasConfig = true;
    }
    if (typeof (mod === null || mod === void 0 ? void 0 : mod.dynamicParams) !== 'undefined') {
        config.dynamicParams = mod.dynamicParams;
        hasConfig = true;
    }
    if (typeof (mod === null || mod === void 0 ? void 0 : mod.dynamic) !== 'undefined') {
        config.dynamic = mod.dynamic;
        hasConfig = true;
    }
    if (typeof (mod === null || mod === void 0 ? void 0 : mod.fetchCache) !== 'undefined') {
        config.fetchCache = mod.fetchCache;
        hasConfig = true;
    }
    if (typeof (mod === null || mod === void 0 ? void 0 : mod.preferredRegion) !== 'undefined') {
...
export async function collectGenerateParams(tree) {
    ...
    while (currentLoaderTree) {
        ...
        const mod = await (isLayout
            ? (_b = (_a = components.layout) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.call(_a)
            : (_d = (_c = components.page) === null || _c === void 0 ? void 0 : _c[0]) === null || _d === void 0 ? void 0 : _d.call(_c));
        if (page) {
...
export async function buildAppStaticPaths({ dir, page, distDir, configFileName, generateParams, isrFlushToDisk, cacheHandler, requestHeaders, maxMemoryCacheSize, fetchCacheKeyPrefix, ppr, ComponentMod, }) {
    ...
    return StaticGenerationAsyncStorageWrapper.wrap(ComponentMod.staticGenerationAsyncStorage, {
        ...
        if (typeof (pageEntry === null || pageEntry === void 0 ? void 0 : pageEntry.getStaticPaths) === 'function') {
            ...
            const buildParams = async (paramsItems = [{}], idx = 0) => {
                const curGenerate = generateParams[idx];
                if (idx === generateParams.length) {
                    return paramsItems;
                }
                if (typeof curGenerate.generateStaticParams !== 'function' &&
                ...
                if (curGenerate.generateStaticParams) {
                    for (const params of paramsItems) {
                        ...
                        for (const item of result) {
                            newParams.push(Object.assign(Object.assign({}, params), item));
                        }
                    }
                }
                if (idx < generateParams.length) {
            ...
            (generate) => { var _a; return ((_a = generate.config) === null || _a === void 0 ? void 0 : _a.dynamicParams) === false; });
            if (!hadAllParamsGenerated) {
                return {
                    paths: undefined,
                    fallback: process.env.NODE_ENV === 'production' && isDynamicRoute(page)
                        ? true
                        : undefined,
    ...
    return isPageStaticSpan
        ...
        if (pathIsEdgeRuntime) {
            const runtime = await getRuntimeContext({
                ...
                edgeFunctionEntry: Object.assign(Object.assign({}, edgeInfo), { wasm: ((_a = edgeInfo.wasm) !== null && _a !== void 0 ? _a : []).map((binding) => (Object.assign(Object.assign({}, binding), { filePath: path.join(distDir, binding.filePath) }))) }),
                name: edgeInfo.name,
                useCache: true,
                distDir,
            });
        ...
        const routeModule = (_b = componentsResult.ComponentMod) === null || _b === void 0 ? void 0 : _b.routeModule;
        if (pageType === 'app') {
            ...
            appConfig = generateParams.reduce((builtConfig, curGenParams) => {
                const { dynamic, fetchCache, preferredRegion, revalidate: curRevalidate, } = (curGenParams === null || curGenParams === void 0 ? void 0 : curGenParams.config) || {};
        ...
        const hasGetInitialProps = !!(Comp === null || Comp === void 0 ? void 0 : Comp.getInitialProps);
        ...
        const config = isClientComponent
            ? {}
...
export async function copyTracedFiles(dir, distDir, pageKeys, appPageKeys, tracingRoot, serverConfig, middlewareManifest, hasInstrumentationHook, staticPages) {
    ...
    const nextConfig = Object.assign(Object.assign({}, serverConfig), { distDir: `./${path.relative(dir, distDir)}` });
    try {
    ...
    async function handleEdgeFunction(page) {
        ...
        await Promise.all([
            ...
            (_a = page.wasm) === null || _a === void 0 ? void 0 : _a.map((file) => handleFile(file.filePath)),
            (_b = page.assets) === null || _b === void 0 ? void 0 : _b.map((file) => handleFile(file.filePath)),
        ]);
    }
