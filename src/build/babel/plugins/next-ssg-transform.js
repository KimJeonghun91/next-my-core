...
function decorateSsgExport(t, path, state) {
    ...
    const addGsspExport = (exportPath) => {
        ...
        const [pageCompPath] = exportPath.replaceWithMultiple([
            t.exportNamedDeclaration(t.variableDeclaration(
            ...
            'var', [t.variableDeclarator(gsspId, t.booleanLiteral(true))]), [t.exportSpecifier(gsspId, gsspId)]),
            exportPath.node,
        ]);
            ...
            Program: {
                enter(path, state) {
                        ...
                        VariableDeclarator(variablePath, variableState) {
                            if (variablePath.node.id.type === 'Identifier') {
                                ...
                                elements.forEach((e) => {
                                    var _a, _b;
                                    let local;
                                    if (((_a = e.node) === null || _a === void 0 ? void 0 : _a.type) === 'Identifier') {
                                        local = e;
                                    }
                                    else if (((_b = e.node) === null || _b === void 0 ? void 0 : _b.type) === 'RestElement') {
                            ...
                            if (specifiers.length) {
                                specifiers.forEach((s) => {
                                    if (isDataIdentifier(t.isIdentifier(s.node.exported)
                                        ...
                                        : s.node.exported.value, exportNamedState)) {
                                        s.remove();
                                    }
                                });
                    ...
                    function sweepFunction(sweepPath) {
                        ...
                        if ((ident === null || ident === void 0 ? void 0 : ident.node) &&
                            refs.has(ident) &&
                            !isIdentifierReferenced(ident)) {
                            ++count;
                            if (t.isAssignmentExpression(sweepPath.parentPath.node) ||
                    ...
                    function sweepImport(sweepPath) {
                        ...
                        if (refs.has(local) && !isIdentifierReferenced(local)) {
                            ++count;
                            sweepPath.remove();
                            if (sweepPath.parent.specifiers
                                .length === 0) {
                                sweepPath.parentPath.remove();
                            }
                        }
