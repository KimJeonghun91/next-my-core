...
export async function exportPages(req, res, path, page, query, htmlFilepath, htmlFilename, ampPath, subFolders, outDir, ampValidatorPath, pagesDataDir, buildExport, isDynamic, hasOrigQueryValues, renderOpts, components, fileWriter) {
    var _a, _b, _c;
    const ampState = {
        ampFirst: ((_a = components.pageConfig) === null || _a === void 0 ? void 0 : _a.amp) === true,
        hasQuery: Boolean(query.amp),
        hybrid: ((_b = components.pageConfig) === null || _b === void 0 ? void 0 : _b.amp) === 'hybrid',
    };
    ...
    const ssgNotFound = renderResult === null || renderResult === void 0 ? void 0 : renderResult.metadata.isNotFound;
    ...
    if (inAmpMode && !renderOpts.ampSkipValidation) {
        ...
        if (!exists) {
            try {
                ampRenderResult = await lazyRenderPagesPage(req, res, page, Object.assign(Object.assign({}, query), { amp: '1' }), renderOpts);
            }
            catch (err) {
                if (!isBailoutToCSRError(err))
                    throw err;
            }
            const ampHtml = ampRenderResult && !ampRenderResult.isNull
                ? ampRenderResult.toUnchunkedString()
                : '';
            if (!renderOpts.ampSkipValidation) {
                await validateAmp(ampHtml, page + '?amp=1');
            }
            await fileWriter(ExportedPagesFiles.AMP_HTML, ampHtmlFilepath, ampHtml, 'utf8');
        }
    }
    const metadata = (renderResult === null || renderResult === void 0 ? void 0 : renderResult.metadata) || (ampRenderResult === null || ampRenderResult === void 0 ? void 0 : ampRenderResult.metadata) || {};
    if (metadata.pageData) {
        const dataFile = join(pagesDataDir, htmlFilename.replace(/\.html$/, NEXT_DATA_SUFFIX));
        await fileWriter(ExportedPagesFiles.DATA, dataFile, JSON.stringify(metadata.pageData), 'utf8');
        if (hybridAmp) {
            await fileWriter(ExportedPagesFiles.AMP_DATA, dataFile.replace(/\.json$/, '.amp.json'), JSON.stringify(metadata.pageData), 'utf8');
        }
    }
    if (!ssgNotFound) {
    ...
    return {
        ampValidations,
        revalidate: (_c = metadata.revalidate) !== null && _c !== void 0 ? _c : false,
        ssgNotFound,
    };
}
