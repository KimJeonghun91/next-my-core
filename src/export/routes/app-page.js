...
export async function exportAppPage(req, res, page, path, pathname, query, renderOpts, htmlFilepath, debugOutput, isDynamicError, fileWriter) {
    ...
    try {
        ...
        if (revalidate === 0) {
            ...
            if (revalidate === 0 && debugOutput && (staticBailoutInfo === null || staticBailoutInfo === void 0 ? void 0 : staticBailoutInfo.description)) {
        ...
        // contain the dynamic data.
            ...
            // payload.
            await fileWriter(ExportedAppPageFiles.PREFETCH_FLIGHT, htmlFilepath.replace(/\.html$/, RSC_PREFETCH_SUFFIX), flightData);
        }
        else {
            // Writing the RSC payload to a file if we don't have PPR enabled.
            await fileWriter(ExportedAppPageFiles.FLIGHT, htmlFilepath.replace(/\.html$/, RSC_SUFFIX), flightData);
        }
        const headers = Object.assign({}, metadata.headers);
        if (fetchTags) {
            headers[NEXT_CACHE_TAGS_HEADER] = fetchTags;
        }
        // Writing static HTML to a file.
        await fileWriter(ExportedAppPageFiles.HTML, htmlFilepath, html !== null && html !== void 0 ? html : '', 'utf8');
        ...
        await fileWriter(ExportedAppPageFiles.META, htmlFilepath.replace(/\.html$/, NEXT_META_SUFFIX), JSON.stringify(meta, null, 2));
        return {
        ...
        if (debugOutput) {
            const { dynamicUsageDescription, dynamicUsageStack } = renderOpts
                .store;
            logDynamicUsageWarning({
                path,
