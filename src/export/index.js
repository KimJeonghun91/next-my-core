...
function setupWorkers(options, nextConfig) {
    ...
    const timeout = (nextConfig === null || nextConfig === void 0 ? void 0 : nextConfig.staticPageGenerationTimeout) || 0;
...
export async function exportAppImpl(dir, options, span) {
    ...
    const customRoutes = ['rewrites', 'redirects', 'headers'].filter((config) => typeof nextConfig[config] === 'function');
    ...
    const pagesManifest = !options.pages &&
        require(join(distDir, SERVER_DIRECTORY, PAGES_MANIFEST));
    ...
    if (!options.buildExport && appRoutePathManifest) {
        for (const [pageName, routePath] of Object.entries(appRoutePathManifest)) {
            ...
            if (isAppPageRoute(pageName) &&
                !(prerenderManifest === null || prerenderManifest === void 0 ? void 0 : prerenderManifest.routes[routePath]) &&
                !(prerenderManifest === null || prerenderManifest === void 0 ? void 0 : prerenderManifest.dynamicRoutes[routePath])) {
    ...
    if (!options.buildExport &&
        ...
        await span
            ...
            .traceAsyncFn(() => recursiveCopy(join(distDir, CLIENT_STATIC_FILES_PATH), join(outDir, '_next', CLIENT_STATIC_FILES_PATH)));
    }
  ...
  Read more: https://nextjs.org/docs/messages/export-image-api`);
    ...
    if (enabledDirectories.app) {
        ...
        if (nextConfig.output === 'export') {
            if (Object.keys(serverActionsManifest.node).length > 0 ||
    ...
    const renderOpts = Object.assign(Object.assign({ previewProps: prerenderManifest === null || prerenderManifest === void 0 ? void 0 : prerenderManifest.preview, buildId, nextExport: true, assetPrefix: nextConfig.assetPrefix.replace(/\/$/, ''), distDir, dev: false, basePath: nextConfig.basePath, trailingSlash: nextConfig.trailingSlash, canonicalBase: ((_a = nextConfig.amp) === null || _a === void 0 ? void 0 : _a.canonicalBase) || '', ampSkipValidation: ((_b = nextConfig.experimental.amp) === null || _b === void 0 ? void 0 : _b.skipValidation) || false, ampOptimizerConfig: ((_c = nextConfig.experimental.amp) === null || _c === void 0 ? void 0 : _c.optimizer) || undefined, locales: i18n === null || i18n === void 0 ? void 0 : i18n.locales, locale: i18n === null || i18n === void 0 ? void 0 : i18n.defaultLocale, defaultLocale: i18n === null || i18n === void 0 ? void 0 : i18n.defaultLocale, domainLocales: i18n === null || i18n === void 0 ? void 0 : i18n.domains, disableOptimizedLoading: nextConfig.experimental.disableOptimizedLoading, 
        // Exported pages do not currently support dynamic HTML.
        supportsDynamicHTML: false, crossOrigin: nextConfig.crossOrigin || '', optimizeCss: nextConfig.experimental.optimizeCss, nextConfigOutput: nextConfig.output, nextScriptWorkers: nextConfig.experimental.nextScriptWorkers, optimizeFonts: nextConfig.optimizeFonts, largePageDataBytes: nextConfig.experimental.largePageDataBytes, serverActions: nextConfig.experimental.serverActions, serverComponents: enabledDirectories.app, nextFontManifest: require(join(distDir, 'server', `${NEXT_FONT_MANIFEST}.json`)), images: nextConfig.images }, (enabledDirectories.app
        ? {
            serverActionsManifest,
        }
        : {})), { strictNextHead: !!nextConfig.experimental.strictNextHead, deploymentId: nextConfig.experimental.deploymentId, experimental: {
    ...
    const filteredPaths = exportPaths.filter((route) => exportPathMap[route]._isAppDir ||
        // Remove API routes
        ...
        if (hasApiRoutes || hasMiddleware) {
            if (nextConfig.output === 'export') {
                Log.warn(yellow(`Statically exporting a Next.js application via \`next export\` disables API routes and middleware.`) +
                    `\n` +
                    yellow(`This command is meant for static-only hosts, and is` +
                        ' ' +
    ...
    const results = await Promise.all(filteredPaths.map(async (path) => {
        ...
        const result = await pageExportSpan.traceAsyncFn(async () => {
            ...
            return await exportPage({
                ...
                pagesDataDir,
                renderOpts,
                ampValidatorPath: ((_a = nextConfig.experimental.amp) === null || _a === void 0 ? void 0 : _a.validator) || undefined,
    ...
    for (const { result, path } of results) {
        ...
        if (result.ampValidations) {
            for (const validation of result.ampValidations) {
                ...
                hadValidationError || (hadValidationError = validation.result.errors.length > 0);
            }
        }
        if (options.buildExport) {
            // Update path info by path.
            const info = (_d = collector.byPath.get(path)) !== null && _d !== void 0 ? _d : {};
            ...
            // Update durations.
            const durations = (_e = collector.byPage.get(page)) !== null && _e !== void 0 ? _e : {
    ...
    if (!options.buildExport && prerenderManifest) {
        await Promise.all(Object.keys(prerenderManifest.routes).map(async (route) => {
            ...
            const distPagesDir = join(pagePath, 
