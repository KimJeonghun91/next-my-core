...
export function defaultHead(inAmpMode = false) {
    const head = [_jsx("meta", { charSet: "utf-8" })];
    if (!inAmpMode) {
        head.push(_jsx("meta", { name: "viewport", content: "width=device-width" }));
    }
    return head;
}
    ...
    return headChildrenElements
        ...
        if (process.env.NODE_ENV !== 'development' &&
            ...
            if (c.type === 'link' &&
                ...
                const newProps = Object.assign({}, (c.props || {}));
...
function Head({ children }) {
    ...
    return (_jsx(Effect, { reduceComponentsToState: reduceComponents, headManager: headManager, inAmpMode: isInAmpMode(ampState), children: children }));
}
export default Head;
