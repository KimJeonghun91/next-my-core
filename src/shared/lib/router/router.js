...
function getMiddlewareData(source, response, options) {
    ...
    if (rewriteTarget) {
        if (rewriteTarget.startsWith('/') ||
            ...
            return Promise.all([
                ...
                if (isDynamicRoute(as) ||
                    (!rewriteHeader &&
                        pages.includes(normalizeLocalePath(removeBasePath(as), options.router.locales)
                            .pathname))) {
                ...
                const resolvedHref = !pages.includes(fsPathname)
                    ? resolveDynamicRoute(normalizeLocalePath(removeBasePath(parsedRewriteTarget.pathname), options.router.locales).pathname, pages)
                    : fsPathname;
        ...
        const pathname = formatNextPathnameInfo(Object.assign(Object.assign({}, getNextPathnameInfo(src.pathname, { nextConfig, parseData: true })), { defaultLocale: options.router.defaultLocale, buildId: '' }));
...
const manualScrollRestoration = process.env.__NEXT_SCROLL_RESTORATION &&
...
function fetchNextData({ dataHref, inflightCache, isPrefetch, hasMiddleware, isServerRender, parseJSON, persistCache, isBackground, unstable_skipClientCache, }) {
    ...
    const getData = (params) => {
        ...
        return fetchRetry(dataHref, isServerRender ? 3 : 1, {
            headers: Object.assign({}, isPrefetch ? { purpose: 'prefetch' } : {}, isPrefetch && hasMiddleware ? { 'x-middleware-prefetch': '1' } : {}),
            method: (_a = params === null || params === void 0 ? void 0 : params.method) !== null && _a !== void 0 ? _a : 'GET',
        })
            .then((response) => {
            ...
            return response.text().then((text) => {
                ...
                if (!response.ok) {
                    ...
                    if (response.status === 404) {
                        if ((_a = tryToParseAsJSON(text)) === null || _a === void 0 ? void 0 : _a.notFound) {
                            return {
                                dataHref,
    ...
    return (inflightCache[cacheKey] = getData(isBackground ? { method: 'HEAD' } : {}));
}
...
class Router {
    constructor(pathname, query, as, { initialProps, pageLoader, App, wrapApp, Component, err, subscription, isFallback, locale, locales, defaultLocale, domainLocales, isPreview, }) {
            ...
            if (process.env.__NEXT_SCROLL_RESTORATION) {
                if (manualScrollRestoration) {
                    if (this._key !== key) {
                        ...
                        try {
                            sessionStorage.setItem('__next_scroll_' + this._key, JSON.stringify({ x: self.pageXOffset, y: self.pageYOffset }));
                        }
                        catch (_a) { }
        ...
        if (process.env.__NEXT_CLIENT_ROUTER_FILTER_ENABLED) {
            ...
            const dynamicFilterData = process.env
        ...
        this.isReady = !!(self.__NEXT_DATA__.gssp ||
        ...
        if (typeof window !== 'undefined') {
            ...
            if (!as.startsWith('//')) {
                ...
                this._initialMatchesMiddlewarePromise = matchesMiddleware({
                    ...
                    this.changeState('replaceState', matches
                        ? asPath
            ...
            for (const curAs of [as, resolvedAs]) {
                if (curAs) {
                    ...
                    if (asNoSlash !==
                        ...
                        matchesBflStatic =
                            matchesBflStatic ||
                                !!((_a = this._bfl_s) === null || _a === void 0 ? void 0 : _a.contains(asNoSlash)) ||
                                !!((_b = this._bfl_s) === null || _b === void 0 ? void 0 : _b.contains(asNoSlashLocale));
                        for (const normalizedAS of [asNoSlash, asNoSlashLocale]) {
                            ...
                            for (let i = 0; !matchesBflDynamic && i < curAsParts.length + 1; i++) {
                                ...
                                if (currentPart && ((_c = this._bfl_d) === null || _c === void 0 ? void 0 : _c.contains(currentPart))) {
    ...
    async change(method, url, as, options, forcedScroll) {
        var _a, _b, _c, _d, _e, _f, _g, _j, _k, _l;
        if (!isLocalURL(url)) {
        ...
        let shouldResolveHref = isQueryUpdating ||
        ...
        if (process.env.__NEXT_I18N_SUPPORT) {
            ...
            if (process.env.__NEXT_I18N_SUPPORT) {
                ...
                if (!((_a = this.locales) === null || _a === void 0 ? void 0 : _a.includes(nextState.locale))) {
            ...
            if (process.env.__NEXT_I18N_SUPPORT) {
                ...
                if (!didNavigate &&
                    ...
                    handleHardNavigation({
                        url: `http${detectedDomain.http ? '' : 's'}://${detectedDomain.domain}${addBasePath(`${nextState.locale === detectedDomain.defaultLocale
                            ? ''
                            : `/${nextState.locale}`}${asNoBasePath === '/' ? '' : asNoBasePath}` || '/')}`,
                        router: this,
                    });
                    // this was previously a return but was removed in favor
                    // of better dead code elimination with regenerator runtime
        ...
        if (!isQueryUpdating && this.onlyAHashChange(cleanedAs) && !localeChange) {
            ...
            this.changeState(method, url, as, Object.assign(Object.assign({}, options), { scroll: false }));
            if (scroll) {
        ...
        if ((_b = this.components[pathname]) === null || _b === void 0 ? void 0 : _b.__appRouter) {
            handleHardNavigation({ url: as, router: this });
            return new Promise(() => { });
        }
        const isMiddlewareRewrite = !!(parsedAsPathname &&
            route !== parsedAsPathname &&
            (!isDynamicRoute(route) ||
                !getRouteMatcher(getRouteRegex(route))(parsedAsPathname)));
        // we don't attempt resolve asPath when we need to execute
        // middleware as the resolving will occur server-side
        const isMiddlewareMatch = !options.shallow &&
        ...
        try {
            ...
            if ('route' in routeInfo && isMiddlewareMatch) {
                ...
                if (isDynamicRoute(pathname)) {
                    const prefixedAs = !routeProps.shallow && routeInfo.resolvedAs
                        ? routeInfo.resolvedAs
                        : addBasePath(addLocale(new URL(as, location.href).pathname, nextState.locale), true);
            ...
            if (isQueryUpdating &&
                ...
                ((_d = (_c = self.__NEXT_DATA__.props) === null || _c === void 0 ? void 0 : _c.pageProps) === null || _d === void 0 ? void 0 : _d.statusCode) === 500 &&
                ((_e = routeInfo.props) === null || _e === void 0 ? void 0 : _e.pageProps)) {
                // ensure statusCode is still correct for static 500 page
                // when updating query information
                routeInfo.props.pageProps.statusCode = 500;
            }
            // shallow routing is only allowed for same page URL changes.
            const isValidShallowRoute = options.shallow && nextState.route === ((_f = routeInfo.route) !== null && _f !== void 0 ? _f : route);
            const shouldScroll = (_g = options.scroll) !== null && _g !== void 0 ? _g : (!isQueryUpdating && !isValidShallowRoute);
            const resetScroll = shouldScroll ? { x: 0, y: 0 } : null;
            const upcomingScrollState = forcedScroll !== null && forcedScroll !== void 0 ? forcedScroll : resetScroll;
            // the new state that the router gonna set
            const upcomingRouterState = Object.assign(Object.assign({}, nextState), { route,
                pathname,
                query, asPath: cleanedAs, isFallback: false });
            ...
            if (isQueryUpdating && isErrorRoute) {
                ...
                if (this.pathname === '/_error' &&
                    ((_k = (_j = self.__NEXT_DATA__.props) === null || _j === void 0 ? void 0 : _j.pageProps) === null || _k === void 0 ? void 0 : _k.statusCode) === 500 &&
                    ((_l = routeInfo.props) === null || _l === void 0 ? void 0 : _l.pageProps)) {
                    // ensure statusCode is still correct for static 500 page
                    // when updating query information
                    routeInfo.props.pageProps.statusCode = 500;
                }
            ...
            const canSkipUpdating = isQueryUpdating &&
                !upcomingScrollState &&
                !readyStateChange &&
                !localeChange &&
    ...
    async getRouteInfo({ route: requestedRoute, pathname, query, as, resolvedAs, routeProps, locale, hasMiddleware, isPreview, unstable_skipClientCache, isQueryUpdating, isMiddlewareRewrite, isNotFound, }) {
        ...
        try {
            ...
            let cachedRouteInfo = existingInfo &&
            ...
            let data = isQueryUpdating && !isMiddlewareRewrite
                ? null
            ...
            handleCancelled();
            if (((_a = data === null || data === void 0 ? void 0 : data.effect) === null || _a === void 0 ? void 0 : _a.type) === 'redirect-internal' ||
                ((_b = data === null || data === void 0 ? void 0 : data.effect) === null || _b === void 0 ? void 0 : _b.type) === 'redirect-external') {
                return data.effect;
            }
            if (((_c = data === null || data === void 0 ? void 0 : data.effect) === null || _c === void 0 ? void 0 : _c.type) === 'rewrite') {
                ...
                if (!isQueryUpdating || pages.includes(resolvedRoute)) {
                    ...
                    query = Object.assign(Object.assign({}, query), data.effect.parsedAs.query);
                    resolvedAs = removeBasePath(normalizeLocalePath(data.effect.parsedAs.pathname, this.locales)
                        .pathname);
                    ...
                    if (routeProps.shallow &&
                        ...
                        return Object.assign(Object.assign({}, existingInfo), { route });
                    }
                }
            }
            if (isAPIRoute(route)) {
            ...
            const wasBailedPrefetch = (_d = data === null || data === void 0 ? void 0 : data.response) === null || _d === void 0 ? void 0 : _d.headers.get('x-middleware-skip');
            ...
            const { props, cacheKey } = await this._getData(async () => {
                if (shouldFetchData) {
                    ...
                    const dataHref = (data === null || data === void 0 ? void 0 : data.dataHref)
                        ? data.dataHref
    ...
    async prefetch(url, asPath = url, options = {}) {
        ...
        const locale = typeof options.locale !== 'undefined'
        ...
        const data = process.env.__NEXT_MIDDLEWARE_PREFETCH === 'strict'
            ? null
            : await withMiddlewareEffects({
        ...
        await Promise.all([
            this.pageLoader._isSsg(route).then((isSsg) => {
                return isSsg
                    ? fetchNextData({
                        dataHref: (data === null || data === void 0 ? void 0 : data.json)
                            ? data === null || data === void 0 ? void 0 : data.dataHref
                            : this.pageLoader.getDataHref({
                                href: url,
                                asPath: resolvedAs,
                                locale: locale,
