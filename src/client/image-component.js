...
function handleLoading(img, placeholder, onLoadRef, onLoadingCompleteRef, setBlurComplete, unoptimized) {
    ...
    p.catch(() => { }).then(() => {
        ...
        if (onLoadRef === null || onLoadRef === void 0 ? void 0 : onLoadRef.current) {
            ...
            let prevented = false;
            let stopped = false;
            onLoadRef.current(Object.assign(Object.assign({}, event), { nativeEvent: event, currentTarget: img, target: img, isDefaultPrevented: () => prevented, isPropagationStopped: () => stopped, persist: () => { }, preventDefault: () => {
                    prevented = true;
            ...
            if (img.getAttribute('data-nimg') === 'fill') {
                ...
                if (img.parentElement) {
                    ...
                    if (!valid.includes(position)) {
                        warnOnce(`Image with src "${origSrc}" has "fill" and parent element with invalid "position". Provided "${position}" should be one of ${valid
                            .map(String)
                            .join(',')}.`);
                    }
                }
...
const ImageElement = forwardRef((_a, forwardedRef) => {
    var { src, srcSet, sizes, height, width, decoding, className, style, fetchPriority, placeholder, loading, unoptimized, fill, onLoadRef, onLoadingCompleteRef, setBlurComplete, setShowAltText, onLoad, onError } = _a, rest = __rest(_a, ["src", "srcSet", "sizes", "height", "width", "decoding", "className", "style", "fetchPriority", "placeholder", "loading", "unoptimized", "fill", "onLoadRef", "onLoadingCompleteRef", "setBlurComplete", "setShowAltText", "onLoad", "onError"]);
    return (_jsx("img", Object.assign({}, rest, getDynamicProps(fetchPriority), { 
        // It's intended to keep `loading` before `src` because React updates
        // props in order which causes Safari/Firefox to not lazy load properly.
        // See https://github.com/facebook/react/issues/25883
        loading: loading, width: width, height: height, decoding: decoding, "data-nimg": fill ? 'fill' : '1', className: className, style: style, 
        ...
        sizes: sizes, srcSet: srcSet, src: src, ref: useCallback((img) => {
            if (forwardedRef) {
...
function ImagePreload({ isAppRouter, imgAttributes, }) {
    const opts = Object.assign({ as: 'image', imageSrcSet: imgAttributes.srcSet, imageSizes: imgAttributes.sizes, crossOrigin: imgAttributes.crossOrigin, referrerPolicy: imgAttributes.referrerPolicy }, getDynamicProps(imgAttributes.fetchPriority));
    ...
    return (_jsx(Head, { children: _jsx("link", Object.assign({ rel: "preload", 
...
export const Image = forwardRef((props, forwardedRef) => {
