...
export function pageBootrap(assetPrefix) {
    ...
    return hydrate({ beforeRender: displayContent }).then(() => {
        ...
        addMessageListener((payload) => {
            ...
            if ('action' in payload) {
                ...
                if (payload.event === HMR_ACTIONS_SENT_TO_BROWSER.MIDDLEWARE_CHANGES) {
                ...
                else if (payload.event === HMR_ACTIONS_SENT_TO_BROWSER.CLIENT_CHANGES) {
                ...
                else if (payload.event === HMR_ACTIONS_SENT_TO_BROWSER.SERVER_ONLY_CHANGES) {
                    const { pages } = payload;
                    ...
                    if (!router.clc && pages.includes(router.pathname)) {
                        ...
                        buildIndicatorHandler === null || buildIndicatorHandler === void 0 ? void 0 : buildIndicatorHandler.show();
                        const clearIndicator = () => buildIndicatorHandler === null || buildIndicatorHandler === void 0 ? void 0 : buildIndicatorHandler.hide();
                        router
                            .replace(router.pathname +
                            '?' +
                            String(assign(urlQueryToSearchParams(router.query), new URLSearchParams(location.search))), router.asPath, { scroll: false })
                            .catch(() => {
