...
class Container extends React.Component {
    ...
    componentDidMount() {
        ...
        if (router.isSsr &&
            ...
            router
                ...
                '?' +
                String(assign(urlQueryToSearchParams(router.query), new URLSearchParams(location.search))), asPath, {
                // @ts-ignore
    ...
    render() {
        if (process.env.NODE_ENV === 'production') {
            ...
            return _jsx(ReactDevOverlay, { children: this.props.children });
        }
    }
}
...
function renderApp(App, appProps) {
    return _jsx(App, Object.assign({}, appProps));
}
function AppContainer({ children, }) {
    ...
    return (_jsx(Container, { fn: (error) => 
        // TODO: Fix disabled eslint rule
        // eslint-disable-next-line @typescript-eslint/no-use-before-define
        renderError({ App: CachedApp, err: error }).catch((err) => console.error('Error rendering page: ', err)), children: _jsx(AppRouterContext.Provider, { value: adaptedForAppRouter, children: _jsx(SearchParamsContext.Provider, { value: adaptForSearchParams(router), children: _jsx(PathnameContextProviderAdapter, { router: router, isAutoExport: (_a = self.__NEXT_DATA__.autoExport) !== null && _a !== void 0 ? _a : false, children: _jsx(PathParamsContext.Provider, { value: adaptForPathParams(router), children: _jsx(RouterContext.Provider, { value: makePublicRouterInstance(router), children: _jsx(HeadManagerContext.Provider, { value: headManager, children: _jsx(ImageConfigContext.Provider, { value: process.env
                                        .__NEXT_IMAGE_OPTS, children: children }) }) }) }) }) }) }) }));
}
const wrapApp = (App) => (wrappedAppProps) => {
    const appProps = Object.assign(Object.assign({}, wrappedAppProps), { Component: CachedComponent, err: initialData.err, router });
    return _jsx(AppContainer, { children: renderApp(App, appProps) });
};
...
function renderError(renderErrorProps) {
    ...
    return pageLoader
        ...
        return (lastAppProps === null || lastAppProps === void 0 ? void 0 : lastAppProps.Component) === ErrorComponent
            ? import('../pages/_error')
                .then((errorModule) => {
        ...
        return Promise.resolve(((_a = renderErrorProps.props) === null || _a === void 0 ? void 0 : _a.err)
            ? renderErrorProps.props
            : loadGetInitialProps(App, appCtx)).then((initProps) => 
        // TODO: Fix disabled eslint rule
        // eslint-disable-next-line @typescript-eslint/no-use-before-define
        doRender(Object.assign(Object.assign({}, renderErrorProps), { err, Component: ErrorComponent, styleSheets, props: initProps })));
    });
}
...
function doRender(input) {
    ...
    const appProps = Object.assign(Object.assign({}, props), { Component,
        err,
        router });
    ...
    onStart();
    const elem = (_jsxs(_Fragment, { children: [_jsx(Head, { callback: onHeadCommit }), _jsxs(AppContainer, { children: [renderApp(App, appProps), _jsx(Portal, { type: "next-route-announcer", children: _jsx(RouteAnnouncer, {}) })] })] }));
    // We catch runtime errors using componentDidCatch which will trigger renderError
    renderReactElement(appElement, (callback) => (_jsx(Root, { callbacks: [callback, onRootCommit], children: process.env.__NEXT_STRICT_MODE ? (_jsx(React.StrictMode, { children: elem })) : (elem) })));
...
async function render(renderingProps) {
    ...
    try {
        ...
        await renderError(Object.assign(Object.assign({}, renderingProps), { err: renderErr }));
    }
}
        ...
        if (mod && mod.reportWebVitals) {
            onPerfEntry = ({ id, name, startTime, value, duration, entryType, entries, attribution, }) => {
                ...
                const webVitals = {
                    ...
                    label: entryType === 'mark' || entryType === 'measure'
                        ? 'custom'
                        : 'web-vital',
                };
                if (attribution) {
