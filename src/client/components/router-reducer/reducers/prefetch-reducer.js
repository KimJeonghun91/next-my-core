...
export function prefetchReducer(state, action) {
    ...
    if (cacheEntry) {
        ...
        if (cacheEntry.kind === PrefetchKind.TEMPORARY) {
            state.prefetchCache.set(prefetchCacheKey, Object.assign(Object.assign({}, cacheEntry), { kind: action.kind }));
        }
        /**
         * if the prefetch action was a full prefetch and that the current cache entry wasn't one, we want to re-prefetch,
         * otherwise we can re-use the current cache entry
         **/
        if (!(cacheEntry.kind === PrefetchKind.AUTO &&
            action.kind === PrefetchKind.FULL)) {
            return state;
        }
    }
    // fetchServerResponse is intentionally not awaited so that it can be unwrapped in the navigate-reducer
    const serverResponse = prefetchQueue.enqueue(() => fetchServerResponse(url, 
