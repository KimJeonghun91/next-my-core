...
function addRefetchToLeafSegments(newCache, currentCache, flightSegmentPath, treePatch, data) {
    ...
    const segmentPathsToFill = generateSegmentsFromPatch(treePatch).map((segment) => [...flightSegmentPath, ...segment]);
...
function navigateReducer_noPPR(state, action) {
    ...
    if (!prefetchValues) {
        const data = fetchServerResponse(url, state.tree, state.nextUrl, state.buildId, 
        ...
        const newPrefetchValue = {
            ...
            kind: process.env.NODE_ENV === 'development'
                ? PrefetchKind.AUTO
    ...
    return data.then(([flightData, canonicalUrlOverride, postponed]) => {
        ...
        for (const flightDataPath of flightData) {
            ...
            if (newTree !== null) {
                ...
                let applied = applyFlightData(currentCache, cache, flightDataPath, (prefetchValues === null || prefetchValues === void 0 ? void 0 : prefetchValues.kind) === 'auto' &&
                ...
                if ((!applied &&
                    ...
                    postponed) {
                    applied = addRefetchToLeafSegments(cache, currentCache, flightSegmentPath, treePatch, 
                ...
                for (const subSegment of generateSegmentsFromPatch(treePatch)) {
                    ...
                    if (scrollableSegmentPath[scrollableSegmentPath.length - 1] !==
                        DEFAULT_SEGMENT_KEY) {
...
function navigateReducer_PPR(state, action) {
    ...
    const { treeAtTimeOfPrefetch, data } = prefetchValues;
    prefetchQueue.bump(data);
    return data.then(([flightData, canonicalUrlOverride, _postponed]) => {
        // we only want to mark this once
        if (prefetchValues && !prefetchValues.lastUsedTime) {
            // important: we should only mark the cache node as dirty after we unsuspend from the call above
            prefetchValues.lastUsedTime = Date.now();
        ...
        for (const flightDataPath of flightData) {
            ...
            if (newTree !== null) {
                ...
                flightDataPath.length === 3) {
                    ...
                    const isPrefetchStale = prefetchEntryCacheStatus === PrefetchCacheEntryStatus.stale;
                    const task = updateCacheNodeOnNavigation(currentCache, currentTree, prefetchedTree, seedData, head, isPrefetchStale);
