...
export function updateCacheNodeOnNavigation(oldCacheNode, oldRouterState, newRouterState, prefetchData, prefetchHead, isPrefetchStale) {
    ...
    for (let parallelRouteKey in newRouterStateChildren) {
        ...
        const oldCacheNodeChild = oldSegmentMapChild !== undefined
        ...
        else if (oldSegmentChild !== undefined &&
...
function finishTaskUsingDynamicDataPayload(task, serverRouterState, dynamicData, dynamicHead) {
    ...
    for (const parallelRouteKey in serverRouterState) {
        ...
        if (taskChild !== undefined) {
            const taskSegment = taskChild.route[0];
            if (matchSegment(serverRouterStateChild[0], taskSegment) &&
...
function createPendingCacheNode(routerState, prefetchData, prefetchHead, isPrefetchStale) {
    ...
    for (let parallelRouteKey in routerStateChildren) {
        ...
        const prefetchDataChild = prefetchDataChildren !== null
...
function finishPendingCacheNode(cacheNode, taskState, serverState, dynamicData, dynamicHead) {
    ...
    for (let parallelRouteKey in taskStateChildren) {
        ...
        const cacheNodeChild = segmentMapChild !== undefined
            ? segmentMapChild.get(taskSegmentKeyChild)
            : undefined;
        if (cacheNodeChild !== undefined) {
            if (serverStateChild !== undefined &&
