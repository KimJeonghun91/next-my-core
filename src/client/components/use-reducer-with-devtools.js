...
function useReducerWithReduxDevtoolsImpl(initialState) {
    ...
    useEffect(() => {
        ...
        if (enabledRef.current === undefined &&
    ...
    const sync = useCallback((resolvedState) => {
        if (devtoolsConnectionRef.current) {
            devtoolsConnectionRef.current.send({ type: 'RENDER_SYNC' }, normalizeRouterState(resolvedState));
        }
    }, []);
    return [state, dispatch, sync];
}
export const useReducerWithReduxDevtools = typeof window !== 'undefined'
