...
function onReport(metric) {
    ...
    if (process.env.NODE_ENV === 'production' &&
        ...
        const body = {
            ...
            id: metric.id,
            page: (_a = window.__NEXT_DATA__) === null || _a === void 0 ? void 0 : _a.page,
            href: initialHref,
...
export default (onPerfEntry) => {
    if (process.env.__NEXT_ANALYTICS_ID) {
        ...
        isRegistered = true;
        const attributions = process.env
            .__NEXT_WEB_VITALS_ATTRIBUTION;
        for (const webVital of WEB_VITALS) {
            try {
                let mod;
                if (process.env.__NEXT_HAS_WEB_VITALS_ATTRIBUTION) {
                    if (attributions === null || attributions === void 0 ? void 0 : attributions.includes(webVital)) {
