...
import { RESTART_EXIT_CODE, checkNodeDebugType, getDebugPort, getMaxOldSpaceSize, getNodeOptionsWithoutInspect, getPort, printAndExit, } from '../server/lib/utils';
...
const handleSessionStop = async (signal) => {
    if (child === null || child === void 0 ? void 0 : child.pid)
        child.kill(signal !== null && signal !== void 0 ? signal : 0);
    if (sessionStopHandled)
        return;
    sessionStopHandled = true;
    if ((child === null || child === void 0 ? void 0 : child.pid) && child.exitCode === null && child.signalCode === null) {
    ...
    try {
        ...
        let pagesDir = !!traceGlobals.get('pagesDir');
        let appDir = !!traceGlobals.get('appDir');
        if (typeof traceGlobals.get('pagesDir') === 'undefined' ||
...
process.on('exit', () => child === null || child === void 0 ? void 0 : child.kill('SIGKILL'));
const nextDev = async (args) => {
    ...
    async function preflight(skipOnReboot) {
        ...
        if (sassVersion && nodeSassVersion) {
            Log.warn('Your project has both `sass` and `node-sass` installed as dependencies, but should only use one or the other. ' +
        ...
        if (!skipOnReboot) {
            ...
            if (dependencies['@next/font'] ||
                (devDependencies['@next/font'] &&
                    devDependencies['@next/font'] !== 'workspace:*')) {
                const command = getNpxCommand(dir);
                Log.warn('Your project has `@next/font` installed as a dependency, please use the built-in `next/font` instead. ' +
    ...
    const distDir = path.join(dir, (_a = config.distDir) !== null && _a !== void 0 ? _a : '.next');
    ...
    async function startServer(options) {
        return new Promise((resolve) => {
            ...
            child = fork(startServerPath, {
                stdio: 'inherit',
                env: Object.assign(Object.assign({}, defaultEnv), { TURBOPACK: process.env.TURBOPACK, NEXT_PRIVATE_WORKER: '1', NODE_EXTRA_CA_CERTS: options.selfSignedCertificate
                        ? options.selfSignedCertificate.rootCA
                        : defaultEnv.NODE_EXTRA_CA_CERTS, NODE_OPTIONS }),
            });
            child.on('message', (msg) => {
                if (msg && typeof msg === 'object') {
                    if (msg.nextWorkerReady) {
                        child === null || child === void 0 ? void 0 : child.send({ nextWorkerOptions: options });
                    }
    ...
    const runDevServer = async (reboot) => {
        try {
            if (!!args['--experimental-https']) {
                Log.warn('Self-signed certificates are currently an experimental feature, use at your own risk.');
                let certificate;
                ...
                await startServer(Object.assign(Object.assign({}, devServerOptions), { selfSignedCertificate: certificate }));
            }
            else {
